...
set_other_groups()
{
   ...
   config_foreach add_other_group "groups" "$1" "$2" #比對策略組
}
...
yml_groups_set()
{
   ...
   config_list_foreach "$section" "other_group" set_other_groups "$name" #加入其他策略組
   config_foreach yml_servers_add "servers" "$name" "$type" #加入伺服器節點
