...
description: 'OpenClash 使用者功能指南。用於回答使用者關於 OpenClash 外掛如何啟用/關閉各項功能的問題，包括：執行模式切換、代理開關、DNS 設定、流量控制、存取控制黑白名單、IPv6 開關、規則/GEO 更新、自動重啟、儀表盤設定、訂閱管理、覆寫設定等。每個選項均標註了對應的 UCI 設定項、修改的 Mihomo YAML 設定段、以及觸發的指令碼。Use when user asks how to enable, disable, configure, or troubleshoot any OpenClash feature on OpenWrt.'
instructions: |
  ...
  1. When users report any issue (cannot access internet, DNS failure, startup failure, etc.), FIRST ask them to generate a debug log — either via LuCI (執行日誌 → 產生日誌) or SSH (`/usr/share/openclash/openclash_debug.sh`). The debug log contains dependency checks, config, firewall rules, and system info in one step.
  2. If the debug log is insufficient to pinpoint the issue, give users precise CLI commands from 第七部分 (診斷指令與 CLI 參考), ask them to run on the router and paste back the output.
...
>
> **排查優先順序（從快到慢，逐層遞進）**：
> ① **先要日誌** — 使用者報告問題時，首先讓使用者產生除錯日誌（包含依賴檢查、設定、防火牆規則、系統資訊等 20+ 章節）
> ② **日誌不足時給指令** — 對照第七部分決策樹，給精確 CLI 指令讓使用者執行
> ③ **確定根因後給路徑** — 定位問題後，給 LuCI 操作路徑指導修復設定
> ④ **仍未解決查外部** — 查 Issues / 原始碼 / Mihomo Wiki

| 原則 | 說明 |
|------|------|
| **主動查證，不猜測** | 遇到本文件未覆蓋的 Mihomo 設定欄位或實現細節時，**禁止編造**。必須使用工具主動查詢外部資源（Mihomo Wiki / Meta-Docs / Mihomo 核心原始碼 / OpenClash 原始碼 / Smart 核心原始碼），整理後告知使用者。詳見文件末尾「超出本文件範圍的查詢」節。 |
| **查原始碼，不只查文件** | 當使用者詢問"為什麼某選項不生效"、"底層實現邏輯是什麼"時，不能僅依賴 [Mihomo Wiki] 和 [Meta-Docs] 的設定文件。必須進一步查閱 [Mihomo 核心原始碼](https://github.com/MetaCubeX/mihomo/tree/Alpha)、[OpenClash 原始碼](https://github.com/vernesong/OpenClash/tree/dev) 和 [Smart 核心原始碼](https://github.com/vernesong/mihomo/tree/Alpha) 中的對應指令碼/函式，理解實際執行邏輯。 |
| **先要日誌，不盲猜** | 使用者報告任何異常（無法上網、DNS 異常、啟動失敗、節點不通等）時，**第一步總是先讓使用者產生除錯日誌**，而非猜測或直接給診斷指令。除錯日誌一鍵包含依賴檢查、執行狀態、防火牆規則、系統資訊等 20+ 章節，比逐條執行診斷指令高效得多。產生方式：① **LuCI 頁面**：「執行日誌」→「產生日誌」按鈕；② **SSH 指令**：`/usr/share/openclash/openclash_debug.sh`（輸出 `/tmp/openclash_debug.log`）。拿到日誌後對照「日誌與錯誤資訊速查」和第七部分決策樹進行診斷。 |
| **日誌不足再給指令** | 僅當除錯日誌不足以定位問題時，才按第七部分的診斷決策樹給使用者精確的 CLI 診斷指令。優先使用 🟢 安全查詢指令，對 🟡/🔴 指令附帶風險說明。使用者執行後貼上輸出，AI 分析結果決定下一步。 |
| **設定路徑，修復給步驟** | 功能設定（如何開啟/關閉/設定選項）和問題修復 → 給出 LuCI Web 介面操作路徑（如「服務 → OpenClash → 外掛設定 → 流量控制」），而非指令列。僅在使用者明確要求 CLI 操作或 LuCI 不可用時才提供終端指令。 |
| **解釋原理，不只給步驟** | 說明設定選項背後的工作原理（如防火牆規則鏈、YAML 轉換邏輯），幫助使用者理解後再操作，降低誤操作風險。 |
| **引用來源** | 當資訊來自外部查詢（Mihomo Wiki、原始碼、Issues 等），在回覆末尾註明來源，讓使用者知道資訊的權威性。 |
| **查 Issues，不閉門造車** | 當使用者遇到的功能問題在本文件中未覆蓋，或報錯資訊在錯誤速查表中無比對項時，**必須主動搜尋 Issues** 查詢是否存在相同或相似的問題：① 外掛設定/訂閱/防火牆/UI 相關問題 → 搜尋 [OpenClash Issues](https://github.com/vernesong/OpenClash/issues)；② 核心級問題（代理協定/TUN/DNS 解析/規則引擎等 Mihomo 核心行為） → 搜尋 [Mihomo Issues](https://github.com/MetaCubeX/mihomo/issues)。優先參考：**作者/維護者的回覆**（OpenClash 標有 Owner 標籤的 vernesong；Mihomo 標有 Contributor/Collaborator 標籤的回覆）——代表官方立場或已知 bug；**高贊反應（👍）的社群回覆**——代表經過驗證的有效方案；**同類問題中的診斷指令**（如 `nft list set`、`dig`、`uci show` 等）——可直接複用於使用者的問題排查。搜尋時使用使用者報錯中的關鍵錯誤資訊或功能描述作為關鍵詞。 |

**核心資源速查**:

| 資源 | URL | 用途 |
|------|-----|------|
| Mihomo Wiki | `https://wiki.metacubex.one/config/` | Mihomo YAML 設定欄位文件 |
| Meta-Docs | `https://github.com/MetaCubeX/Meta-Docs` | Mihomo 設定欄位權威參考 |
| OpenClash Issues | `https://github.com/vernesong/OpenClash/issues` | 搜尋外掛側已知問題、社群方案、作者回復 |
| Mihomo Issues | `https://github.com/MetaCubeX/mihomo/issues` | 搜尋核心側已知問題（代理協定/TUN/DNS/規則引擎等） |
| Mihomo 核心原始碼 | `https://github.com/MetaCubeX/mihomo/tree/Alpha` | Mihomo 核心實現（代理協定/規則引擎/DNS/TUN 等 Go 原始碼） |
...
> **AI 行為指引**: 當使用者報告啟動失敗、功能異常時，AI 應**先讓使用者產生除錯日誌**（LuCI「執行日誌」→「產生日誌」或 SSH `openclash_debug.sh`），然後對照日誌中的 `#===== 依賴檢查 =====#` 段檢查依賴完整性。對於缺失的依賴，指導使用者在 LuCI 的「系統 → 軟體套件」中搜尋安裝。
>
...
OpenClash 依賴以下軟體套件，由 `opkg`/`apk` 在安裝時自動拉取。若手動解除安裝了其中某個包，會導致對應功能異常。

| 相依套件 | 作用 | 缺失症狀 | 安裝指令 (LuCI) |
|--------|------|----------|-----------------|
| `dnsmasq-full` | DNS 轉發與劫持（必須用 full 版，非精簡版） | DNS 劫持失效、用戶端無法解析網域名稱 | 「系統→軟體套件」搜尋 `dnsmasq-full` |
| `bash` | 所有 Shell 指令碼的直譯器 | 啟動指令碼執行失敗 | 搜尋 `bash` |
| `curl` | HTTP/HTTPS 下載（訂閱、GEO、Dashboard） | 訂閱更新失敗、GEO 下載報錯 | 搜尋 `curl` |
| `ca-bundle` | CA 憑證包（curl HTTPS 驗證） | curl SSL 憑證錯誤 | 搜尋 `ca-bundle` |
| `ip-full` | 策略路由和 ipset/nftset 操作 | 路由表操作失敗 | 搜尋 `ip-full` |
| `ruby` | YAML 解析與設定產生 | `yml_change.sh` 報錯、設定無法產生 | 搜尋 `ruby` |
...
| `ruby-pstore` | Ruby 持久化儲存（訂閱快取） | 訂閱設定快取異常 | 搜尋 `ruby-pstore` |
| `kmod-tun` | TUN 虛擬網路卡核心模組 | TUN 模式無法啟動 | 搜尋 `kmod-tun` |
| `kmod-inet-diag` | 程式名診斷（PROCESS-NAME 規則） | PROCESS-NAME 規則不生效 | 搜尋 `kmod-inet-diag` |
| `unzip` | 解壓 Dashboard/GEO 等壓縮檔 | Dashboard 下載後無法載入 | 搜尋 `unzip` |
| `luci-compat` | LuCI >= 19.07 相容層（新版 LuCI 必裝） | LuCI 頁面佈局錯亂、JS 報錯 | 搜尋 `luci-compat` |

### 二、防火牆相關依賴（按 fw4/fw3 自動區分）

| 環境 | 相依套件 | 作用 | 缺失症狀 | 安裝指令 (LuCI) |
|------|--------|------|----------|-----------------|
...
| **fw3 (iptables)** | `kmod-ipt-extra` | iptables 擴充套件比對模組 | 高階規則比對失敗 | 搜尋 `kmod-ipt-extra` |
...
> **診斷方法**: 先在 LuCI 的「執行日誌」頁面產生除錯日誌，在日誌的依賴檢查段確認 dnsmasq 版本。如需手動確認，可在路由器終端執行 `dnsmasq --version | head -1`。
> 如果不是，在 LuCI 的「系統 → 軟體套件」中解除安裝 `dnsmasq` 然後安裝 `dnsmasq-full`。
...
| 故障現象 | 可能原因 | LuCI 排查路徑 |
|----------|----------|--------------|
| 啟動失敗，日誌顯示 "Ruby Works Abnormally" | `ruby` 或 `ruby-yaml` 未安裝/損壞 | 「系統→軟體套件」確認 `ruby`、`ruby-yaml`、`ruby-psych` 已安裝 |
| TUN 模式啟動報錯 "tun module not found" | `kmod-tun` 未安裝或核心版本不相容 | 「系統→軟體套件」安裝 `kmod-tun`，注意核心版本是否相容 |
| 訂閱更新報 SSL 憑證錯誤 | `ca-bundle` 未安裝或過期 | 「系統→軟體套件」安裝/更新 `ca-bundle` |
| DNS 劫持不生效 | 安裝了精簡版 `dnsmasq` 而非 `dnsmasq-full` | 「系統→軟體套件」解除安裝 `dnsmasq`，安裝 `dnsmasq-full` |
| UDP 流量無法代理（fw4） | `kmod-nft-tproxy` 未安裝 | 「系統→軟體套件」安裝 `kmod-nft-tproxy` |
| Dashboard 頁面白屏/404 | `unzip` 未安裝導致儀表盤解壓失敗 | 「系統→軟體套件」安裝 `unzip`，然後重新下載儀表盤 |
| LuCI 頁面佈局錯亂、按鈕無回應 | `luci-compat` 未安裝 | 「系統→軟體套件」安裝 `luci-compat` |
| 程式名規則 (PROCESS-NAME) 不生效 | `kmod-inet-diag` 未安裝 | 「系統→軟體套件」安裝 `kmod-inet-diag` |
| 更新外掛後某些包丟失 | 更新過程中包被意外移除 | 更新指令碼會自動修復，如仍未恢復，手動安裝缺失包 |

> **通用依賴診斷方法**: 在 LuCI 的「執行日誌」頁面點選「產生日誌」，然後在日誌的 `#===== 依賴檢查 =====#` 段檢視所有相依套件的狀態（已安裝/未安裝）。將此日誌提供給技術支援時也包含完整的依賴資訊。
...
│       │  UCI 讀寫    │            │                              │
│       ▼             ▼            ▼                              │
│  /etc/config/openclash  — UCI 設定檔案 (所有選項持久化在此)       │
│       │                                                         │
...
│       │                                                         │
│       ▼ 產生 / 覆寫                                              │
│  /etc/openclash/config/*.yaml  — 原始訂閱設定                    │
│  /etc/openclash/*.yaml          — 經指令碼處理後的執行設定          │
│  /etc/openclash/overwrite/     — 覆寫模組檔案                    │
│  /etc/openclash/custom/        — 使用者自訂規則/DNS/防火牆指令碼    │
│       │                                                         │
│       ▼                                                         │
...
| 路徑 | 作用 |
|------|------|
| `/etc/config/openclash` | UCI 設定檔案，所有 LuCI 選項持久化在此 |
| `/etc/openclash/` | OpenClash 工作目錄（核心、GEO 資料、Chnroute 列表） |
| `/etc/openclash/config/` | 原始訂閱設定存放目錄（`.yaml` 檔案，經 `yml_change.sh` 處理後產生 `/etc/openclash/<name>.yaml` 執行設定） |
| `/etc/openclash/overwrite/` | 覆寫模組檔案（INI 格式，定義自訂 YAML 覆蓋） |
| `/etc/openclash/custom/` | 使用者自訂檔案（規則列表、DNS 策略、Hosts、防火牆指令碼、Sniffer 設定等） |
...
- **UCI 設定根**: `openclash` (所有選項均在 `uci show openclash` 可見)
- **Mihomo 執行時 API**: `http://路由器LAN_IP:9090` — 部分動態選項透過 PATCH `/configs` 熱生效。注意：API 位址是**路由器 LAN 口 IP**，不是 127.0.0.1（核心監聽 `0.0.0.0`，但 LuCI 後端透過 `127.0.0.1` 直連核心 API）
- **核心啟動指令碼**: `/etc/init.d/openclash {start|stop|restart|reload|enable|disable}`
- **自訂檔案目錄**: `/etc/openclash/custom/` — 存放使用者自訂規則/DNS/防火牆指令碼
...
│
├─ 第1步: 讀取設定
│   ├─ overwrite_file()     → 遍歷 config_overwrite 條目，產生 /tmp/yaml_overwrite.sh
│   ├─ get_config()         → 讀取所有 UCI 選項為 Shell 變數
│   ├─ config_choose()      → 選擇活動的 YAML 設定檔案
...
│   │   ├─ enable_custom_clash_rules → 從 *.list 檔案注入自訂規則
│   │   └─ auto_smart_switch → 將 url-test/load-balance 組改為 smart 型別
│   │
│   └─ ③ /tmp/yaml_overwrite.sh  → 來自覆寫模組 [Overwrite] 段的自訂指令碼
│
├─ 第4步: 啟動核心
│   └─ procd 啟動 clash -d /etc/openclash -f <config.yaml>
│       ├─ respawn 設定: 重試 5 次, 間隔 3s, 逾時 300s
...
└─ 第6步: 定時任務 + 守護程式
    ...
    └─ start_watchdog()      → 啟動守護程式
        ├─ openclash_watchdog.sh        → 核心存活監控
        └─ openclash_streaming_unlock.lua → 流媒體解鎖守護
```

**停止流程** (`stop_service()`):
1. 備份策略組狀態歷史 → 2. `revert_firewall()` 清除防火牆規則 → 3. kill clash + streaming unlock 程式 → 4. `revert_dnsmasq()` 恢復 DNS → 5. `del_cron()` 清除定時任務
...
> "如何驗證防火牆規則是否生效"、"TUN 模式下某協定不通"），AI 應**先讓使用者產生除錯日誌**
...
> DNS 劫持規則使用 `meta nfproto {ipv4}` 限制僅比對 IPv4 流量；IPv6 DNS 劫持在 IPv6 段獨立處理。
...
# 3. LAN 白名單未符合 RETURN (lan_ac_mode=1, 同時有 IP+MAC 白名單時兩者均未符合才 RETURN)
...
```bash
# TUN 模式 extras: forward 鏈額外比對 oifname utun 覆蓋經 TUN 轉發的流量
...
```bash
# 1. IPv6 區域網路繞過 (localnetwork6: ::/128, ::1/128, fe80::/10, ff00::/8 等)
...
- **繞過檢查仍然生效**：ICMP 規則之前的 localnetwork/WAN-AC/LAN-AC/china_ip_route 等 RETURN 規則同樣適用於 ICMP——被比對的 ICMP 包會跳過標記規則。
- **路由器自身 ICMP**：當 `router_self_proxy=1` 時，路由器發出的 ping 在 `openclash_mangle_output` 鏈中同樣被標記。

**2. TUN 模式（`en_mode_tun=1`）**:

ICMP echo-request 在 `openclash_mangle` 鏈中被標記 fwmark，隨後透過策略路由進入 TUN 虛擬網路卡：
...
- **ICMP 被代理**：TUN 模式下所有標記 fwmark 的流量（包括 ICMP）被策略路由導向 `utun` 虛擬網路卡，由 Mihomo 核心的 TUN 協定棧處理。
- **Mihomo 核心設定**：TUN 模式下 Mihomo 支援兩個 ICMP 相關選項：
  - `icmp-timeout`（預設自動）：ICMP 連線逾時時間（秒）
  - `disable-icmp-forwarding`（預設 false）：設為 `true` 可停用 TUN 的 ICMP 轉發（ping 將不被代理）

**3. Fake-IP 非 TUN 模式的 Ping 阻斷**:

**僅在 Fake-IP 非 TUN 模式下**（`en_mode=fake-ip`, `en_mode_tun` 為空），對 Fake-IP 位址段（預設 `198.18.0.0/16`）的 ping 會被防火牆**顯式 REJECT**：
...
# OUTPUT 鏈 — 阻止路由器發出對 Fake-IP 的 ping（排除 OpenClash 自身程式 skgid=65534）
...
這是因為在非 TUN 模式下，Fake-IP 位址沒有對應的 TCP/UDP 重定向路徑（TCP 走 REDIRECT、UDP 走 TPROXY，但 ICMP 都不到達核心），發往這些位址的 ping 無意義且會干擾網路診斷。OUTPUT 鏈排除 `skgid=65534` 是為了避免影響 OpenClash 自身程式的內部通訊。

> **TUN 模式下的區別**：Fake-IP **TUN 模式不新增這些 REJECT 規則**。因為 ICMP 經策略路由進入 TUN 虛擬網路卡後，由核心的 `skipPingForwardingByAddr()` 判斷——若目標是 Fake-IP，核心返回偽造 echo-reply（~0ms 虛假延遲），不產生實際網路流量。
...
IPv6 非 TUN 模式下 ICMPv6 **不被標記也不被代理**。IPv6 Fake-IP 位址範圍的 ping 在**非 TUN 的 IPv6 模式下**被 REJECT（返回 `icmpv6 admin-prohibited`），條件為 `$ipv6_mode -ne 2 -a $ipv6_mode -ne 3`。TUN/Mix 模式下的 IPv6 Fake-IP ping 同樣由核心的 `skipPingForwardingByAddr()` 處理（偽造回覆）。
...
> **實用提示**：如果使用者發現 ping 不通但網頁正常，首先確認不是 Fake-IP **非 TUN** 模式下在 ping 被代理的網域名稱（Fake-IP 返回 `198.18.x.x`，防火牆直接 REJECT）。Fake-IP TUN/Mix 模式下 ping Fake-IP 位址會返回虛假 ~0ms 延遲。非 Fake-IP 的真實 IP ping 在 TUN/Mix 模式下走 DIRECT 直連，延遲反映的是區域網路品質。

**核心側 ICMP 處理機制**（`listener/sing_tun/prepare.go` — Mihomo TUN 監聽器）:

當 ICMP echo-request 經策略路由進入 TUN 虛擬網路卡後，Mihomo 核心按以下優先順序處理：

1. **目標是 Fake-IP 位址**（`resolver.IsFakeIP(addr)`） → 返回 `nil, nil`，核心用**偽造的 echo-reply** 回覆。上層看到 "ping 成功" 但實際未經過網路，延遲顯示為虛假的 ~0ms
2. **目標是 TUN 介面自身 IP**（`inet4_address` / `inet6_address` 範圍內） → 同上，偽造回覆
3. **`disable-icmp-forwarding: true`** → 所有 ICMP 均偽造回覆
4. **以上均不滿足**（真實 IP 且未停用轉發） → 透過 `ping.ConnectDestination()` 以 **DIRECT 模式**發出真實 ICMP 包，等待真實 reply。延遲為區域網路到目標的實際 RTT
5. **ICMP 逾時**: 預設 10 秒（`sing.go` 常量），可透過 `icmp-timeout` 自訂

> **關鍵結論**: TUN 模式下 ping 的處理分兩種情況——目標是 Fake-IP → 虛假 0ms 延遲；目標是真實 IP → DIRECT 直連延遲。ping **始終不經過代理節點**，這與 TCP/UDP 流量（經代理轉發）的行為不同。

---

#### F. 高階流量控制 (`firewall_lan_ac_traffic`) — 按裝置/協定/埠/DSCP 精確控制

> **UCI 設定路徑**: `config firewall_lan_ac_traffic` 段，透過「外掛設定 → 黑白名單 → 高階流量控制」設定。
...
| `src_ip` | string | IP/CIDR 或 `localnetwork` | 源 IP 位址（`localnetwork` 表示比對所有區域網路裝置） |
| `src_port` | string | 埠範圍 (如 `0-65535`) | 源埠範圍 |
| `proto` | string | `tcp`/`udp`/`both` | 比對的協定 |
| `target` | string | `return`/`accept`/`drop` | 動作：`return`=跳過代理(預設)/`accept`=放行/`drop`=丟棄(等效return) |
| `dscp` | string | DSCP 值 (如 `46`) | DSCP 標記比對（需 iptables DSCP 模組，fw4 無需額外模組） |
| `family` | string | `ipv4`/`ipv6`/`both` | IP 協定族 |
| `interface` | string | 介面名 (如 `br-lan`) | 入介面比對 |
| `user` | string | UID | 按使用者 ID 比對（僅 OUTPUT 鏈） |
| `comment` | string | 描述文字 | 規則註釋/標識 |

**規則插入位置** (fw4):

每條規則根據協定和模式被插入到以下鏈的最前面：
...
> **注意事項**:
> - 所有規則自動排除 Fake-IP 位址範圍（`ip daddr != {<fakeip_range>}`），確保 Fake-IP 流量不受影響。
> - `target=drop` 在防火牆規則中實際執行為 `return`（跳過代理），區別在於 `drop` 在策略路由/旁路由鏈中也執行 `return`。
> - `user` 欄位僅對 OUTPUT 鏈生效（路由器自身出站流量），入站流量不支援 UID 比對。
> - DSCP 比對在 fw3 (iptables) 環境下需要 `iptables-mod-extra`（提供 DSCP 模組），如不可用會輸出警告並跳過 DSCP 規則。
...
if iptables 不支援 owner 模組; then
    owner="-m mark --mark 0x1a0a"     # 回退: 按 fwmark 比對
...
| **`disable_udp_quic`** (停用 QUIC / Disable QUIC) | `1` | 全部模式在 INPUT/FORWARD 鏈插入 QUIC REJECT 規則 (`udp dport 443`，根據 `china_ip_route`/`china_ip6_route` 比對或排除中國 IP)。TUN 模式額外在 `forward oifname utun` 插入同規則以覆蓋經 utun 轉發的流量。IPv6 同樣處理。規則觸發僅依賴 `disable_udp_quic`，與 `enable_udp_proxy`/`enable_v6_udp_proxy` 無關。Mihomo 核心自身 QUIC（如 Hysteria 節點、DNS h3）不受影響——核心出站走 OUTPUT 鏈，不在規則範圍內 |
| **`lan_ac_mode`** (區域網存取控制模式 / LAN Access Control Mode) | `0` (黑名單) | 建立 `lan_ac_black_ips`/`lan_ac_black_macs`/`lan_ac_black_ipv6s` set，比對到的 RETURN 跳過代理。DNS 劫持規則同步過濾黑名單裝置 |
| | `1` (白名單) | 建立 `lan_ac_white_ips`/`lan_ac_white_macs`/`lan_ac_white_ipv6s` set，**未符合**的 RETURN 跳過代理（反邏輯）。DNS 劫持規則僅對白名單裝置生效 |
...
| **`intranet_allowed`** (僅允許內網 / Only Intranet Allowed) | `1` | IPv4: 建立 `openclash_wan_input` 鏈，REJECT 來自 WAN 口對全部服務埠的存取。IPv6: 建立 `openclash_wan6_input` 鏈。服務埠: `$proxy_port`(7892)、`$tproxy_port`(7895)、`$cn_port`(9090)、`$http_port`(7890)、`$socks_port`(7891)、`$mixed_port`(7893)、`$dns_port`(7874) |
...
| **`skip_proxy_address`** (繞過伺服器位址 / Skip Proxy Address) | `1` | 看門狗定時呼叫 `skip_proxies_address()` 透過核心 API 解析代理節點 `server` 位址並加入 `localnetwork` nft set，複用鏈首 RETURN 規則跳過代理，防止代理巢狀 |
| **`enable_redirect_dns`** (本機 DNS 劫持 / Redirect Local DNS Setting) | `1` | IPv4+IPv6 在 `dstnat` 插入 DNS 53 埠 REDIRECT 規則到 dnsmasq 埠。AC 黑白名單裝置過濾。`router_self_proxy=1` 時新增 OUTPUT DNS 劫持 |
| | `2` | 建立 `openclash_dns_redirect` 鏈，IPv4+IPv6 DNS 流量直接 DNAT 到 `dns_port`(7874)。同樣支援 AC 過濾和 OUTPUT 劫持 |
| **`local_network_pass`** (本機 IPv4 繞過位址 / Local IPv4 Network Bypassed List) | 已設定 | 建立 `localnetwork` nft set (預設: 0.0.0.0/8, 127.0.0.0/8, 10.0.0.0/8, 169.254.0.0/16, 192.168.0.0/16, 224.0.0.0/4, 240.0.0.0/4, 172.16.0.0/12, 100.64.0.0/10)，在所有鏈規則首位比對 RETURN。自訂檔案可覆蓋預設值。WAN 介面 IP 自動加入 |
| **`chnroute_pass`** (繞過指定區域 IPv4 黑名單 / Chnroute Bypassed List) | 已設定 | 建立 `china_ip_route_pass` nft set / ipset，配合 dnsmasq 將指定網域名稱解析的 IP 加入 set。防火牆規則中作為 `china_ip_route` 的排除條件（確保這些 IP 不被繞行規則跳過）。僅在 `enable_redirect_dns != 2` 時生效（依賴 dnsmasq） |
...
| **`local_network6_pass`** (本機 IPv6 繞過位址 / Local IPv6 Network Bypassed List) | 已設定 | 建立 IPv6 `localnetwork6` nft set (預設包含 ::/128, ::1/128, fe80::/10, ff00::/8 等)，IPv6 鏈中比對本機 IPv6 段 RETURN。WAN IPv6 介面位址自動加入 |
...
| | TUN 模式 / Mix 模式 | ICMP 標記 fwmark 後經策略路由進入 TUN 虛擬網路卡，由 TUN 核心處理（真實 IP → DIRECT 直連延遲，Fake-IP → 偽造回覆 ~0ms）；可透過 Mihomo 的 `disable-icmp-forwarding` 停用 |
| **`firewall_lan_ac_traffic`** (高階流量控制 / Advanced Traffic Control) | 已設定 (UCI section) | 透過 `lan_ac_traffic` UCI sections 按裝置/協定/埠/DSCP 精確控制，每條規則插入到對應鏈的最前面 (position 0)，優先順序高於所有其他規則。支援 `return`(跳過代理)/`accept`(放行)/`drop`。詳見上文 F 節 |
...
# 將 openclash_custom_chnroute_pass.list 中的網域名稱加入 set
# 對 china_ip_route_pass UCI 列表中的網域名稱加入 set
...
- 建立 `china_ip_route_pass` ipset/nftset
- 將 chnroute_pass 網域名稱加入 set: `ipset=/domain.com/china_ip_route_pass` 或 `nftset=/domain.com/4#inet#fw4#china_ip_route_pass`
- 效果: DNS 解析這些網域名稱時加入 set 便於在比對時繞過（而非被 chnroute 影響）

---

## 日誌與錯誤資訊速查

> **AI 行為指引**: 當使用者提供日誌報錯資訊時，AI 應首先在以下表格中查詢比對的錯誤關鍵字，
...
> - **核心日誌**（第十、十一類）：由 Mihomo 核心（Go 程式）產生，含 `level=debug/info/warning/error/fatal` 標記，同樣寫入 `/tmp/openclash.log`。`level=fatal` 會導致核心程式退出。可在 LuCI「執行日誌」頁面檢視，或在「執行狀態」頁面看到 `OpenClash Start Failed` 提示。

### 一、核心啟動與執行錯誤

| 錯誤關鍵字 | 問題位置 | 原因 | 排查方法 |
|-----------|---------|------|----------|
| `Ruby Works Abnormally, Please Check The Ruby Library Depends!` (Ruby 依賴異常) | 「執行狀態」啟動流程 | `ruby` 或 `ruby-yaml` 套件未安裝/損壞 | 「系統→軟體套件」安裝 `ruby`、`ruby-yaml`、`ruby-psych` |
| `Unable To Parse Config File` (設定檔案校驗失敗) | 「執行狀態」啟動流程 | YAML 設定檔案語法錯誤或 age 解密失敗 | 「設定管理」頁面點選 Edit 檢查 YAML 語法 |
| `Core Start Failed, Please Check The Log Infos!` (核心啟動失敗) | 「執行狀態」啟動流程 | 核心程式未能啟動 | 「執行狀態」檢視核心版本是否正確；「執行日誌」產生除錯日誌 |
| `Core Initial Configuration Timeout` (核心初始化逾時) | 「執行狀態」啟動流程 | 核心 API 在 300 秒內未就緒 | 檢查 `/tmp/openclash.log` 中核心日誌；確認「覆寫設定→常規」的 cn_port 未被佔用 |
| `TUN Interface Start Failed` (TUN 介面啟動失敗) | 「執行狀態」啟動流程 | TUN 虛擬網路卡建立失敗 | 「系統→軟體套件」確認 `kmod-tun` 已安裝 |
| `【{module}】module not found` (核心模組未找到) | 「執行狀態」啟動流程 | 核心模組未安裝/未載入（tun/tproxy 等） | 「系統→軟體套件」安裝對應的 kmod 包 |
| `LAN IP Address Get Error` (LAN IP 取得失敗) | 「執行狀態」啟動流程 | LAN 介面 IP 無效或 `ip-full` 套件缺失（舊核心 4.4.x 常見 br-lan 網橋無 IP） | 「外掛設定→流量控制」選擇正確的 LAN 介面名稱（如 `br-lan`）；「系統→軟體套件」安裝 `ip-full`；終端 `ip address show br-lan` 確認存在 IPv4 位址；嘗試切換執行模式為混合模式 |
| `OpenClash Now Disabled, Need Start From Luci Page` (外掛未啟用) | 「執行狀態」啟動流程 | 外掛被停用（enable=0） | 「執行狀態」頁面點選啟動開關 |

### 二、訂閱與設定更新錯誤

| 錯誤關鍵字 | 問題位置 | 原因 | 排查方法 |
|-----------|---------|------|----------|
| `Config File Subscribed Failed` (訂閱設定下載失敗) | 「設定訂閱」更新流程 | 訂閱 URL 下載失敗（curl 錯誤） | 「設定訂閱」檢查訂閱 URL 是否正確；確認網路連通性 |
| `Config File Tested Faild` (設定檔案測試失敗) | 「設定訂閱」更新流程 | 下載的 YAML 未透過 `clash -t` 驗證 | 「設定管理」頁面 Edit 檢查 YAML 語法；檢視 `/tmp/openclash.log` |
| `Updated Config Has No Proxy Field` (設定無節點欄位) | 「設定訂閱」更新流程 | 訂閱設定中無 `proxies` 和 `proxy-providers` 欄位 | 檢查訂閱源是否有效；可能訂閱已過期 |
| `Filter Proxies Failed` (節點篩選失敗) | 「設定訂閱」更新流程 | 節點關鍵字過濾正則異常 | 「設定訂閱」檢查 keyword/ex_keyword 格式 |
| `Ruby Works Abnormally` (Ruby 異常) | 「設定訂閱」更新流程 | Ruby 環境異常導致訂閱處理失敗 | 「系統→軟體套件」重灌 `ruby`、`ruby-yaml` |
| `Config File Format Validation Failed` (設定檔案格式校驗失敗) | 「執行狀態」啟動流程 | YAML 解析後檔案為空/丟失 | 「設定管理」檢查設定目錄許可權和磁碟空間 |

### 三、GEO 與規則更新錯誤

| 錯誤關鍵字 | 問題位置 | 原因 | 排查方法 |
|-----------|---------|------|----------|
| `Download Failed: HTML Response Detected` (下載失敗：檢測到 HTML 回應) | 「外掛設定→GEO 資料庫訂閱」 | CDN 返回的是 HTML 錯誤頁而非 GEO 檔案 | 「覆寫設定→常規」檢查 Github 位址修改 CDN 選項 |
| `Download Failed: File Size Too Small` (下載失敗：檔案過小) | 「外掛設定→GEO 資料庫訂閱」 | 下載檔案 <1KB，內容不完整 | 「外掛設定→GEO 資料庫訂閱」檢查 GEO 自訂 URL 是否正確 |
| `Update Error, Please Try Again Later` (更新失敗，請稍後再試) | 「外掛設定→GEO 資料庫訂閱」 | 網路下載失敗 | 「執行狀態」檢查網路連通性；若使用代理下載，新增直連規則 |
| `Control Panel Unzip Error!` (控制面板解壓失敗) | 「執行狀態」儀表盤切換 | Dashboard 壓縮檔解壓失敗 | 「系統→軟體套件」確認 `unzip` 已安裝 |
...
| 錯誤關鍵字 | 問題位置 | 原因 | 排查方法 |
|-----------|---------|------|----------|
| `Core Version Check Error` (核心版本檢測失敗) | 「版本更新」 | GitHub 不可達，無法取得最新版本資訊 | 「執行狀態」檢查網路連通性；如在大陸，設定 CDN |
| `Core Update Failed` (核心更新失敗，重試 3 次後) | 「版本更新」 | 核心下載/解壓/替換失敗 | 「版本更新」確認快閃記憶體空間和 CPU 架構選擇；「系統 → 軟體套件」檢查磁碟空間 |
| `No Compiled Version Selected` (未選擇編譯版本) | 「版本更新」 | CPU 架構未選擇（core_version=0） | 「版本更新」標籤頁選擇對應的 CPU 架構 |
| `Pre update test failed` (更新前測試失敗，3 次後) | 「版本更新」 | 外掛 IPK/APK 安裝測試失敗 | 手動在「系統→軟體套件」中更新或重灌 luci-app-openclash |
| `OpenClash update failed` (OpenClash 更新失敗) | 「版本更新」 | 外掛安裝徹底失敗 | 包已儲存在 `/tmp/`，手動使用 `opkg install` 或 `apk add` 安裝 |
| `Failed to get version information` (取得版本資訊失敗) | 「版本更新」 | GitHub 版本檢查失敗 | 檢查網路；「覆寫設定→常規」設定 CDN |
...
| `Nameserver Option Must Be Setted, Stop Customing DNS Servers` (Nameserver 未設定) | 「覆寫設定→DNS」 | 自訂 DNS 啟用但未設定任何 nameserver | 「覆寫設定→DNS」新增至少一個 DNS 伺服器 |
| `Fallback-Filter Need fallback of DNS Been Setted` (Fallback-Filter 需要 Fallback DNS) | 「覆寫設定→DNS」 | fallback-filter 需要先設定 fallback DNS | 「覆寫設定→DNS」先新增 fallback 分組的 DNS 伺服器 |
| `DNS Loop Check` (DNS 迴環檢查) | 「覆寫設定→DNS」 | DNS 設定存在迴環風險 | 「覆寫設定→DNS」檢查伺服器列表，避免將 Clash DNS 埠設為其上游 |
...
| `skip invalid Overwrite command` (無效覆寫指令) | 「覆寫設定」覆寫模組 | [Overwrite] 段指令不以 `ruby_` 開頭 | 修正指令語法，使用 `ruby_method_name` 格式 |
...
| `Config File Overwrite Failed` (設定檔案覆寫失敗) | 「覆寫設定」覆寫模組 | 覆寫應用整體失敗 | 檢查所有覆寫設定的語法 |
...
| `Something Wrong While Testing` (流媒體測試失敗) | 「外掛設定→流媒體增強」 | 流媒體測試指令碼執行失敗 | 「執行狀態」確認核心執行中；「外掛設定→流媒體增強」檢查策略組設定 |
...
| `Config file does not exist` (設定檔案不存在) | 「設定管理」 | 設定檔案路徑無效 | 「設定管理」檢查檔名；確認檔案存在於設定列表中 |
| `File size exceeds 10MB limit` (檔案超過 10MB 限制) | 「設定管理」上傳 | 上傳檔案超過 10MB | 減小檔案或拆分上傳 |
| `Cannot delete the last remaining dashboard` (無法刪除最後一個儀表盤) | 「執行狀態」儀表盤切換 | 只剩一個儀表盤時不允許刪除 | 「執行狀態」先下載新的儀表盤再刪除舊的 |
| `Failed to generate age key` (產生 Age 金鑰失敗) | 「設定訂閱」Age 金鑰 | 核心不支援 age keygen | 「版本更新」檢查核心版本；手動產生 age 金鑰 |
| `Failed to calculate public key` (計算公鑰失敗) | 「設定訂閱」Age 金鑰 | 金鑰格式無效 | 驗證 age 金鑰格式（應以 `AGE-SECRET-KEY-` 開頭） |
| `Bad address specified!` (位址無效) | 「執行狀態」連線診斷 | 輸入位址為空或無效 | 輸入有效的主機名或 IP 位址 |
| `OpenClash Start Failed: {msg}` (OpenClash 啟動失敗) | 「執行狀態」 | 核心日誌中出現 fatal/error 級別日誌 | 檢視完整錯誤訊息；「執行日誌」產生除錯日誌 |
| `Access Denied` (無法存取) / `Access Timed Out` (連線逾時) | 「執行狀態」IP 檢測 | 網路連線問題 | 檢查路由器網路連線 |

### 九、YAML 設定處理錯誤

| 錯誤關鍵字 | 問題位置 | 原因 | 排查方法 |
|-----------|---------|------|----------|
| `Load File Failed` (載入檔案失敗) | 「設定管理」設定載入 | Ruby 無法載入設定檔案 | 確認設定檔案存在且許可權正確 |
| `Set Custom DNS Failed` (自訂 DNS 設定失敗) | 「覆寫設定→DNS」 | DNS 覆寫處理失敗 | 檢查「覆寫設定→DNS」中的 DNS 伺服器設定 |
| `Set Fake-IP-Filter Failed` (Fake-IP-Filter 設定失敗) | 「覆寫設定→DNS」 | Fake-IP 過濾器設定異常 | 「覆寫設定→DNS」檢查 Fake-IP-Filter 檔案和模式 |
| `Set Hosts Rules Failed` (Hosts 規則設定失敗) | 「覆寫設定→DNS」 | 自訂 Hosts 格式錯誤 | 「覆寫設定→DNS」檢查 hosts 檔案每行格式 |
| `Set Custom Rules Failed` (自訂規則設定失敗) | 「覆寫設定→規則」 | 自訂規則注入異常 | 「覆寫設定→規則」檢查規則檔案語法 |
...
| `proxy-groups Get Failed` (策略組取得失敗) | 「設定管理」策略組 | 設定中策略組解析異常 | 「設定管理」頁面 Edit 檢查 proxy-groups 段 |

### 十、Ruby YAML 模組錯誤

| 錯誤關鍵字 | 問題位置 | 原因 | 排查方法 |
|-----------|---------|------|----------|
| `Fix short-id values type failed` (short-id 型別修復失敗) | 「設定管理」YAML 處理 | YAML 中 `short-id` 欄位值型別修復時 Psych 解析異常 | 「設定管理」Edit 檢查設定中 `short-id` 欄位的值格式 |
| `YAML overwrite failed:【key: ...】` (YAML 覆寫失敗) | 「覆寫設定」覆寫模組 | 覆寫模組 YAML 合併時發生異常 | 「覆寫設定」檢查 `[YAML]` 段的語法和運算子使用 |
| `YAML overwrite failed:【(match value) => ...】` (YAML 條件覆寫比對失敗) | 「覆寫設定」覆寫模組 | 批次條件更新的 where 比對邏輯異常 | 「覆寫設定」檢查 `key*` 運算子的 where 條件格式和正則 |
| `YAML overwrite failed:【(batch update) => ...】` (YAML 批次更新失敗) | 「覆寫設定」覆寫模組 | 批次條件更新執行時異常 | 「覆寫設定」檢查 `key*` 運算子的 set 子句語法 |
| `Write file failed` (寫檔案失敗) | 「設定管理」YAML 寫入 | YAML 寫入檔案時 I/O 異常 | 檢查磁碟空間和檔案許可權 |
| `Decrypt attempt failed` (解密嘗試失敗) | 「設定訂閱」Age 解密 | Age 加密檔案解密失敗 | 「設定訂閱」檢查 age 金鑰是否正確；驗證加密檔案完整性 |
| `Decrypted content empty or still encrypted` (解密後為空或仍加密) | 「設定訂閱」Age 解密 | Age 解密後內容為空或仍為加密格式 | 「設定訂閱」確認 age 金鑰與加密時使用的金鑰比對 |
| `Encrypt attempt failed` (加密嘗試失敗) | 「設定訂閱」Age 加密 | Age 加密寫入時失敗 | 「設定訂閱」檢查 age 公鑰格式；驗證核心年齡功能 |
| `Encrypted file: decryption failed` (加密檔案解密失敗) | 「設定訂閱」Age 解密 | 所有 age 金鑰嘗試均解密失敗 | 「設定訂閱」檢查所有訂閱的 age 金鑰；可能金鑰不相符 |

### 十一、Mihomo 核心設定解析錯誤（`level=fatal` / `level=error`）

> 以下為 Mihomo 核心在**載入/解析 YAML 設定檔案**時產生的錯誤。`level=fatal` 會導致核心程式退出。
...
| 錯誤詳情 | 設定段 | 修復方法 |
|---------|--------|----------|
| `proxy <N>: missing type` | `proxies` | 在「設定管理」Edit 中給第 N 個代理節點新增 `type:` 欄位（如 `ss`, `vmess`, `trojan` 等） |
| `proxy <N>: unsupport proxy type: <type>` | `proxies` | 代理型別名稱拼寫錯誤或不支援，檢查 `type:` 值是否在 Mihomo 支援列表中 |
| `proxy <name> is the duplicate name` | `proxies` | 兩個代理節點同名，在「設定管理」Edit 中修改其中一個的名稱 |
...
| `<groupName>: use or proxies missing` | `proxy-groups` | 策略組沒有設定 `proxies:` 或 `use:`，至少新增一個 |
...
| `decrypt config error` | 全域性 | Age 加密的設定檔案解密失敗，在「設定訂閱」中檢查 age 金鑰 |
| `configuration file ... is empty` | 全域性 | 設定檔案為空，在「設定管理」中檢查設定是否正常下載 |
...
| `DNS [addr] config with invalid ecs` | `dns` | DNS 伺服器的 ECS 設定格式無效，「覆寫設定→DNS」檢查 DNS 伺服器設定 |
...
> **通用排查**: 在「設定管理」頁面點選 **Download Run** 下載經指令碼處理後的執行時設定，對比原始訂閱檢查 `yml_change.sh` 和覆寫模組產生的 YAML 是否正確。

### 十二、DNS 洩露排查

> **核心驗證方法**：在用戶端執行 `nslookup www.google.com`，應返回：① DNS 伺服器為 OpenWrt 路由器 IP；② 解析結果為 Fake-IP 範圍位址（`198.18.x.x`）。若返回真實 IP 或上游 DNS 非路由器，說明 DNS 解析鏈路異常。正確鏈路應為：`裝置 → Dnsmasq(53埠) → OpenClash(7874埠)`。

| 錯誤關鍵字 | 問題位置 | 原因 | 排查方法 | 來源 |
|-----------|---------|------|----------|------|
| DNS 洩露（ipleak / `ipleak.net` 檢測到國內 DNS） | 「覆寫設定→DNS」 | Redir-Host/Fake-IP 下 nameserver 和 fallback 併發請求，國內 DNS 結果可能被優先採納 | ① Meta 核心建議**放棄 fallback**，僅用 `nameserver-policy` 做 DNS 分流（國內網域名稱→國內 DNS，國外網域名稱→國外 DNS）；② 境外 DNS 位址後加 `#PROXY` 強制走代理（如 `https://1.1.1.1/dns-query#PROXY`）；③ 刪除原設定 YAML 的 `dns:` 段，僅透過「覆寫設定→DNS」管理 DNS 設定避免衝突；④ 將 `proxy-server-nameserver` 設為國內 DNS 避免代理節點網域名稱解析走境外 | [#3843](https://github.com/vernesong/OpenClash/issues/3843) |
| `nameserver-policy` 未生效，DNS 仍走 nameserver | 「覆寫設定→DNS」 | OpenClash 的「覆寫設定→DNS」選項會與訂閱設定的 `dns:` 段合併，可能導致預期外的 DNS 行為 | ① 在「覆寫設定→DNS」啟用「自訂 DNS 設定 (Custom DNS Setting)」後重新設定所有 DNS 規則；② 在「執行日誌」中開啟 Debug 等級觀察實際 DNS 查詢路徑；③ 確認 `default-nameserver` 組的 DNS 伺服器開啟了「節點網域名稱解析」選項 | 同上 |
| DNS 洩露（開啟 IPv6 後出現） | 「覆寫設定→DNS」+「IPv6 設定」 | 電信業者下發的 IPv6 DNS 繞過了 OpenClash 的 DNS 劫持，直接回應用戶端請求（"搶答"） | ① 在 LuCI 的「網路→DHCP/DNS→高階設定」中取消 `過濾 IPv6 AAAA 記錄`；② 在 LAN 介面 DHCP 伺服器 IPv6 設定中**取消「本機 IPv6 DNS 伺服器」**，強制裝置使用路由器 IPv4 位址進行 DNS 解析；③ DHCPv6 服務設為已停用，RA 設為伺服器模式。原理：DNS 請求走 IPv4 通道，流量走 IPv6 通道——IPv4 DNS 同樣可以查詢 AAAA 記錄返回 IPv6 位址 | — |
| 旁路由環境下 DNS 洩露 | 「執行狀態」 | 旁路由裝置未正確指定上游 DNS 為 OpenWrt IP（尤其是 IPv6 DNS 留空） | ① 旁路由裝置必須**手動指定 IPv4 DNS 為 OpenWrt 路由器 IP**；② **IPv6 DNS 必須留空**；③ 若使用 DHCP 分配，確保 DHCP 伺服器不下發 IPv6 DNS 位址 | — |

### 十三、版本更新與下載失敗

| 錯誤關鍵字 | 問題位置 | 原因 | 排查方法 | 來源 |
|-----------|---------|------|----------|------|
| `/tmp/openclash_last_version` 下載失敗 | 「執行日誌」/ 啟動流程 | ① curl SSL 憑證驗證失敗（`BADCERT_CN_MISMATCH` / `self signed certificate`）；② GitHub Raw 網域名稱被 DNS 汙染或不可達；③ curl 逾時（`Operation timed out`）；④ 缺少 `libmbedtls` 庫 | ①「覆寫設定→常規」設定 **Github 位址修改 (github_address_mod)** 為 CDN（推薦 `https://fastly.jsdelivr.net/` 或 `https://testingcf.jsdelivr.net/`）；②「系統→軟體套件」確認 `ca-bundle` 已安裝；③ Fake-IP 模式在「覆寫設定→DNS」的 fake-ip-filter 中排除 `raw.githubusercontent.com`；④ 修改 `/usr/share/openclash/openclash_core.sh` 中 curl 的逾時引數 `-m 60` 改為 `-m 300`；⑤ 終端執行 `opkg install libmbedtls` 修復 curl 庫依賴 | [#2791](https://github.com/vernesong/OpenClash/issues/2791) |
| **更新核心 (Update Core)** 點選後重啟失敗 | 「執行狀態」頁面 | v0.47.052 重啟流程中 stop→start 間隔不足，舊核心程式未完全退出即啟動新核心，觸發「核心啟動失敗」 | ① 更新到 v0.47.054+（已在 Developer 分支修復）；② 臨時解決：編輯 `/etc/init.d/openclash`，在 restart 函式的 stop 和 start 之間加 `sleep 5`；③ 如更新後仍失敗，檢查記憶體是否不足（小型裝置建議增加 swap） | [#4969](https://github.com/vernesong/OpenClash/issues/4969) |
| 升級後依賴檢查異常，無法啟動 | 「執行日誌」啟動流程 | 更新後 `check_mod()` 或依賴檢測邏輯誤報 | ①「執行日誌」產生除錯日誌檢查依賴段；②「系統→軟體套件」確認 `kmod-nft-tproxy`/`kmod-ipt-tproxy` 已安裝；③ 切換 Dev 分支取得最新修復；④ 重灌 `luci-app-openclash` | [#4807](https://github.com/vernesong/OpenClash/issues/4807) |
...
| 錯誤關鍵字 | 問題位置 | 原因 | 排查方法 | 來源 |
|-----------|---------|------|----------|------|
| **向日葵/AnyDesk 等遠端軟體無法連線** | 區域網用戶端 | 遠端軟體網域名稱/QUIC 流量被代理或阻斷 | ①「覆寫設定→規則」新增直連規則：`DOMAIN-SUFFIX,oray.com,DIRECT`、`DOMAIN-SUFFIX,sunlogin.net,DIRECT` 等；② 確認 sniffer `skip-domain` 已包含 `oray.com` 和 `sunlogin.net`（預設已含）；③ 嘗試關閉「外掛設定→流量控制」的 `disable_udp_quic` (停用 QUIC) | [#3229](https://github.com/vernesong/OpenClash/issues/3229) |
| **小米攝像機/智慧家居外網無法存取** | 區域網 IoT 裝置 | IoT 裝置流量被代理導致 NAT 穿透失敗 | ①「外掛設定→黑白名單」新增攝像機 IP 到「不走代理的區域網裝置 IP (LAN Bypassed Host List)」列表；② 確認 sniffer `skip-domain` 包含 `Mijia Cloud`（預設已含）；③「覆寫設定→規則」新增 IoT 網域名稱直連規則：`DOMAIN-SUFFIX,xiaomi.com,DIRECT` | [#2431](https://github.com/vernesong/OpenClash/issues/2431) |
...
| **自訂防火牆規則（開發者選項）不生效** | 「外掛設定→開發者設定」 | 編輯後未重啟或指令碼語法錯誤 | ① 修改 `openclash_custom_firewall_rules.sh` 後需**重啟 OpenClash**（不是過載防火牆）；② 用 `bash -n` 檢查指令碼語法；③「執行日誌」產生除錯日誌檢查是否成功執行（日誌中含自訂指令碼內容） | [#4005](https://github.com/vernesong/OpenClash/issues/4005) |
| **DDNS 服務（如 DDNS-GO）工作異常** | 路由器 DDNS 外掛 | DDNS 服務商 API 網域名稱被錯誤分配 Fake-IP，導致 IP 檢測失敗 | ① 將 DDNS 服務商的 API 網域名稱加入「覆寫設定→DNS」的 Fake-IP-Filter 中（填入網域名稱使其返回真實 IP）；② 常見需排除的網域名稱如 `ddns.oray.com`、`api.cloudflare.com` 等，具體根據所用服務商填寫 | — |
| **Cloudflare Tunnel (Cloudflared) 連線不穩定** | 路由器/內網裝置 | Cloudflared 預設使用 QUIC 連線，而海外 QUIC 流量預設被 OpenClash 阻斷 | ① 規則中已指定 Cloudflare Tunnel 相關網域名稱直連；② 在 Cloudflared 啟動引數中顯式指定 `--protocol http2` 強制使用 HTTP/2（Docker 版：`command: [tunnel, --no-autoupdate, --protocol, http2, run, --token, ${CF_TOKEN}]`） | — |
| **BT/PT 下載流量進入核心** | 下載裝置 | 下載裝置流量未正確分流 | ① 若下載裝置為獨立裝置（如 NAS），在「覆寫設定→規則→自訂規則」中新增 `SRC-IP-CIDR,192.168.1.x/32,DIRECT`；② 若同時啟用了 IPv6，還需新增 IPv6 字尾規則 `SRC-IP-SUFFIX,::a1b2:c3d4,DIRECT`（字尾由 EUI-64 產生，可在裝置上檢視）；③ 非獨立裝置可設定「非標埠」策略組直連來規避 80/443 以外的下載流量 | — |
| **直連網站/APP/小程式打不開** | 區域網用戶端 | 小眾網域名稱未被 geosite:cn 收錄，被誤判為非直連走代理 | ① 臨時方案：將「漏網之魚」策略組設為直連；② 永久方案：在「覆寫設定→規則→自訂規則」中為對應網域名稱新增 `DOMAIN-SUFFIX,xxx.com,DIRECT` 規則；③ 觀察 zashboard 中命中策略組確認分流是否正確 | — |
| **開啟 IPv6 後某些直連存取卡頓** | 區域網用戶端 | IPv6 DNS 搶答或電信業者 IPv6 DNS 不穩定導致解析異常 | ① 停用「覆寫設定→DNS」的「追加上游 DNS」，改為在 NameServer 中手動新增 DoH 伺服器（如 AliDNS）；② 確保 LAN 口未下發 IPv6 DNS 位址 | — |
...
| **Hysteria / Hysteria2 / TUIC 節點連線失敗、斷流、握手逾時** | 核心日誌 `level=error` | ① Linux 核心 ≥6.6 的 quic-go GSO 相容性問題（最常見）；② Hysteria 協定對 `server`/`auth`/`tls`/`password` 欄位設定敏感 | ① **優先嚐試**：「外掛設定→模式設定」開啟**「停用 quic-go GSO (Disable QUIC Go GSO)」**後重啟 OpenClash；② 確認 YAML 中 `type: hysteria` 或 `type: hysteria2` 拼寫正確、埠號正確；③ 檢查節點的 `auth`/`password` 及 TLS 憑證設定是否完整 | — |
| **開啟「繞過中國大陸 IP」後 Google Play 商店無法下載/更新** | 用戶端（Android 裝置） | `services.googleapis.cn` 等 Google 網域名稱被國內 DNS 解析到中國大陸 IP（`220.181.x.x`），被 `china_ip_route` 規則比對後走直連；但 Google 中國伺服器禁止境外 IP（代理節點）存取，導致死迴圈 | **從 DNS 和規則兩方面同時入手**：<br><br>**① DNS 層面** — 在「覆寫設定→DNS→自訂 DNS 設定」中設定 `nameserver-policy` 強制 Google 網域名稱走境外 DNS 解析，寫入 YAML 的 `dns.nameserver-policy` 段：<br>```yaml<br>dns:<br>  nameserver-policy:<br>    '+.services.googleapis.cn': 'https://dns.google/dns-query'<br>    '+.googleapis.cn': 'https://dns.google/dns-query'<br>    '+.xn--ngstr-lra8j.com': 'https://dns.google/dns-query'<br>```<br>也可用 `8.8.8.8` 或 `1.1.1.1` 替代 `https://dns.google/dns-query`。效果：網域名稱解析到 Google 境外 IP（如 `142.250.x.x`），而非國內 `220.181.x.x`。<br><br>**② 規則層面** — 在「覆寫設定→規則→自訂規則」中新增，寫入 YAML 的 `rules` 段：<br>```yaml<br>rules:<br>  - DOMAIN-SUFFIX,services.googleapis.cn,Proxy<br>  - DOMAIN-SUFFIX,googleapis.cn,Proxy<br>  - DOMAIN-SUFFIX,xn--ngstr-lra8j.com,Proxy<br>```<br>其中 `Proxy` 替換為你的代理策略組名。更徹底的方式：`GEOSITE,google,Proxy` 將全部 Google 流量走代理。<br><br>**驗證**：終端執行 `dig services.googleapis.cn @127.0.0.1 -p 7874` 應返回境外 IP；在 zashboard 連線日誌中確認網域名稱命中代理規則。 | [#5074](https://github.com/vernesong/OpenClash/issues/5074) |

### 十五、執行時狀態異常

| 錯誤關鍵字 | 問題位置 | 原因 | 排查方法 | 來源 |
|-----------|---------|------|----------|------|
| **節點正常，突然無法存取外網** | 「執行狀態」一切正常但用戶端無網路 | DNS 劫持失效（dnsmasq 被其他外掛修改）、防火牆規則亂序、TUN 路由表丟失 | ①「執行狀態」確認核心和 DNS 埠正常；② 在「執行日誌」中檢查最近的錯誤；③「執行狀態」點選「Reload Firewall (重置防火牆)」重建規則；④ 檢查是否同時執行其他代理/DNS 外掛（如 AdGuard Home、PassWall、SSR-Plus 等），OpenClash 不能與這些外掛共存 | [#3516](https://github.com/vernesong/OpenClash/issues/3516) |
...
| 錯誤關鍵字 | 問題位置 | 原因 | 排查方法 | 來源 |
|-----------|---------|------|----------|------|
| 旁路由 R2S 等 ARM 裝置 iPhone 待機耗電嚴重 | 區域網 | 代理模式下 ARP 代理或 TUN 模式的 keepalive 導致 iPhone 頻繁被喚醒 | ① 嘗試切換為 Fake-IP 模式；② 關閉「僅允許內網 (Only Intranet Allowed)」以外的 WAN 口存取；③ 主路由 DHCP 下發的閘道器和 DNS 指向旁路由 IP | [#2614](https://github.com/vernesong/OpenClash/issues/2614) |
| 在 Fake-IP 模式下無法使用 UU 加速器等遊戲加速軟體 | 「執行狀態」 | 遊戲加速器需要真實 DNS 解析來最佳化連線，Fake-IP 返回虛擬 IP 導致失效 | ① 在「覆寫設定→DNS」的 fake-ip-filter 中新增加速器相關網域名稱（如 `+.leigod.com`、`+.vivox.com`）；② 將加速器所在裝置的 IP 加入「不走代理的區域網裝置 IP (LAN Bypassed Host List)」 | [#1751](https://github.com/vernesong/OpenClash/issues/1751) |
...
> 資料來源: 前端 JS 同時請求多個後端端點：`/status` (執行狀態、儀表盤設定)、`/toolbar_show` (流量統計)、`/update` (本機設定與已裝版本)、`/last_version` (遠端最新版本)、`/oc_settings` (快捷設定)、`/rule_mode` (代理模式)、`/config_file_list` (設定檔案列表) 等。**版本資訊拆分為兩個端點**：`/update` (action_update) 返回本機設定（corever/release_branch/smart_enable 等）與已裝版本（coremetacv/opcv），**不含遠端最新**；遠端最新版本 `corelv`/`oplv` 由獨立 `/last_version` 端點 (action_last_version) 返回（status 頁「新版本可用」紅點據此顯示），均非 `/status` 端點。`/status` 僅返回執行狀態布林值、儀表盤可用性和 core_type，不包含版本號。
...
| **TUN 模式** | `redir-host-tun` / `fake-ip-tun` | 使用 TUN 虛擬網路卡接管所有流量 |
...
| **網域名稱嗅探 (Sniffer)** | 是否啟用 Mihomo 網域名稱嗅探 | `enable_meta_sniffer` | `action_switch_oc_setting` → 動態修改執行時 YAML `sniffer.enable` |
...
| 操作 | 功能 | 後端路由 |
|------|------|----------|
| **設定檔案選擇器** | 下拉切換當前使用的 YAML 設定 | `action_switch_config` → 更新 `config_path` + 自動重啟 |
| **切換 (Switch)** | 切換到選中的設定 | 同上 |
| **更新設定** | 重新下載訂閱並更新 | `action_update_config` → 呼叫 `openclash.sh` |
| **編輯 (Edit)** | 線上編輯 YAML 設定檔案 | 彈出 `config_edit` 模態框 (基於 CodeMirror，支援原始/執行時檢視切換、合併檢視對比、覆寫卡片欄) |
| **編輯訂閱** | 修改該設定的訂閱引數 | 跳轉到 `config-subscribe-edit` |
| **上傳** | 上傳新的 YAML 設定檔案 | 彈出 `config_upload` 模態框 (支援檔案上傳 + 訂閱連結兩個標籤頁) |
| **重新整理訂閱按鈕** | 手動重新整理當前設定的訂閱資訊 | `/sub_info_get` 端點 |
| **指定 URL 按鈕** | 設定訂閱資訊查詢 URL | `/set_subinfo_url` 端點 |
| **訂閱進度條** | 顯示訂閱流量使用情況 (已用/總量/百分比) | `/sub_info_get` 自動輪詢 |

## 1.6 控制面板卡片

顯示當前 Dashboard 存取位址及 Secret 密碼。對應 UCI:
- `cn_port` — API 埠 (預設 9090)，對應 Mihomo `external-controller`
- `dashboard_password` — API 金鑰，對應 Mihomo `secret`
- `dashboard_forward_domain` / `dashboard_forward_port` / `dashboard_forward_ssl` — 公網存取設定
- 提供 **複製 IP** 和 **複製金鑰** 按鈕

## 1.7 混合代理卡片

顯示 SOCKS5/HTTP 代理位址，可複製或產生 PAC 檔案：
- `mixed_port` (預設 7893), `http_port` (7890), `socks_port` (7891)
- 使用者認證: `authentication` TypedSection 中的 `username`/`password`，對應 Mihomo `authentication` 設定
- 提供 **複製代理位址**、**複製認證資訊**、**產生 PAC 設定** 按鈕

## 1.8 儀表盤入口 (Control Panel)

4 種可選儀表盤：**Dashboard** (Yacd)、**Yacd**、**Metacubexd**、**Zashboard**
- 對應 Mihomo `external-ui` 設定
- 切換觸發: `action_switch_dashboard` → `openclash_download_dashboard.sh`
- 預設儀表盤: UCI `default_dashboard`
- **前端存取位址**（`status.htm` JS 按 3 種場景構造）：
  - **本機存取**（瀏覽器 hostname 比對 LAN IP）：`http://<lan_ip>:<cn_port>/ui/<dashboard>/`
  - **公網存取**（設定了 `dashboard_forward_domain` + `dashboard_forward_port`）：`http[s]://<domain>:<port>/ui/<dashboard>/`（協定由 `dashboard_forward_ssl` 決定）
  - **其他情況**：取當前頁面協定 + `lan_ip` + `cn_port`
...
## 1.11 IP 檢測頁 (IP Address / 存取檢查)

**IP 位址部分 (IP Address)**：
- 並行查詢 4 個 IP 源：UpaiYun、IPIP.NET、IP.SB、IPIFY，每個顯示 IP 位址 + 地理資訊
...
```
 UCI 寫入 → init.d start_service() → get_config() 讀取所有 UCI 變數
         ...
         (修改 YAML 設定)         (iptables/nftables)      (DNS 劫持轉發)
                    │                       │                       │
                    ▼                       ▼                       ▼
...
| `yml_rules_change.sh` | UCI 覆寫 + 自訂規則 | 修改執行 YAML | URL-Test 覆寫、GitHub CDN、自訂規則注入、BT 直連規則 |
| `set_firewall()` | 所有流量控制 UCI | iptables/nftables 規則 | 透明代理、黑白名單存取控制、中國 IP 繞行、QUIC 阻斷、UPNP 排除 |
| `change_dnsmasq()` | DNS 相關 UCI | dnsmasq 設定修改 | DNS 劫持轉發、自訂網域名稱 DNS、chnroute 旁路 |

### 外掛強制覆蓋/停用的設定（使用者不可修改）
...
| `sniffer.force-domain` | `netflix, nflxvideo, amazonaws, media.dssott.com` | 強制嗅探的流媒體網域名稱 |
| `sniffer.skip-domain` | `Mijia Cloud, dlg.io.mi.com, +.oray.com, +.sunlogin.net, +.push.apple.com` | 跳過嗅探的智慧家居/推送網域名稱 |
| `sniffer.force-dns-mapping` | `true` (Redir-Host 時) | Redir-Host 模式下強制 DNS 對映嗅探 |
| `iptables` | **刪除** | 強制移除 iptables 相關設定 |
| `ebpf` | **刪除** | 強制移除 eBPF 相關設定 |
| `auto-redir` | **刪除** | 強制移除 auto-redir（由 OpenClash 防火牆管理） |
| `routing-mark` | `6666` (非自訂標記時) | 固定路由標記值 |
| `external-controller-cors.allow-private-network` | `true` (有 CORS origin 時) | 允許私有網路存取 API |

**有條件預設設定**（僅在使用者未設定時自動新增）：

| 設定 | 預設值 | 條件 |
|------|--------|------|
| `keep-alive-interval` | `15` | 僅當設定中未設定 |
| `keep-alive-idle` | `600` | 僅當設定中未設定 |
| `ntp.enable` | `true` | 僅當設定中未設定 |
| `ntp.server` | `time.apple.com` | 僅當設定中未設定 |
| `ntp.port` | `123` | 僅當設定中未設定 |
| `ntp.interval` | `30` (分鐘) | 僅當設定中未設定 |
| `ntp.write-to-system` | `true` | 僅當設定中未設定 |
...
> **故障排查**：如果啟動日誌提示 "nft_tproxy module not found"，請在 LuCI 的「系統 → 軟體套件」中搜尋安裝 `kmod-nft-tproxy`；提示 "xt_TPROXY module not found"，安裝 `kmod-ipt-tproxy`。TUN 模式還需 `kmod-tun`（同樣在 LuCI 軟體套件頁面安裝）。注意 fw4 環境下應檢查 `nft_tproxy` 而非 `xt_TPROXY`。
...
- **Mihomo 對應設定**: `dns.enhanced-mode` (fake-ip / redir-host)
- **Redir-Host 模式**: DNS 解析在用戶端完成，核心根據 IP 規則分流。適合 BT/PT 下載
- **Fake-IP 模式**: DNS 解析在核心完成，返回虛假 IP (198.18.x.x)，效能更高。規則基於網域名稱比對。**推薦作為日常使用首選**：Fake-IP（增強）模式下 TCP/UDP 均走系統協定棧，效能最優；若出現 NAT 問題可切換為 Fake-IP（混合）模式；若韌體含 Docker 則直接選用 Fake-IP（TUN）模式
- **TUN 模式**: 建立虛擬網路卡，以網路層接管所有流量。對應 Mihomo `tun.enable=true`。需要 `kmod-tun` 核心模組
...
- **Mihomo 對應設定**: `tun.stack`
- **system**: 使用 Linux 系統協定棧，效能和穩定性最好
- **gvisor**: 使用者空間網路協定棧，隔離性更好，避免核心態/使用者態切換
...
- **注意**: Fake-IP 模式即使關閉此選項，網域名稱類 UDP 連線仍會經過核心
...
- **預設**: 0 (關閉)
- **說明**: 旁路由 (旁路閘道器) 模式下無法上網時嘗試開啟。如果裝置沒有 WLAN，建議停用 LAN 介面的橋接選項

### disable_quic_go_gso — 停用 quic-go GSO (Disable QUIC Go GSO)
- **UCI 選項**: `openclash.@openclash[0].disable_quic_go_gso`
- **預設**: 0 (關閉)
- **說明**: Linux 核心 6.6 以上版本遇到 QUIC UDP 問題時嘗試開啟。**Hysteria / Hysteria2 / TUIC 等基於 QUIC 協定的節點出現連線逾時、斷流、握手失敗時，優先嚐試開啟此選項**
- **Mihomo 對應設定**: `disable-quic-go-gso` (全域性 experimental 選項，寫入 YAML 的 `experimental.disable-quic-go-gso: true`)
...
> **AI 行為指引 — QUIC 類節點（Hysteria / Hysteria2 / TUIC）故障排查**: 當使用者報告 Hysteria、Hysteria2、TUIC 等基於 QUIC 協定的節點無法連線、頻繁斷流或握手逾時時，AI 應先讓使用者產生除錯日誌檢視核心日誌中的 QUIC 相關錯誤。如日誌含 `quic-go` / `GSO` / `timeout` 等關鍵字，**優先建議**使用者在「外掛設定→模式設定」中開啟「停用 quic-go GSO (Disable QUIC Go GSO)」選項並重啟 OpenClash。這是 Linux 核心 ≥6.6 上最常見的 QUIC 相容性問題。
>
> **關於 `disable_udp_quic`（停用 QUIC）**：此選項**不會**影響 Mihomo 核心自身的 Hysteria/Hysteria2/TUIC 等 QUIC 出站連線，無需因使用此類節點而關閉該選項。原因：所有模式（TUN/非TUN）下 QUIC REJECT 規則均在 filter INPUT 鏈 + IPv6 TUN 模式下額外在 FORWARD -o utun 鏈，Mihomo 核心自身出站 QUIC 走 OUTPUT 鏈，回覆包的目標埠為臨時埠（非 443），均不命中攔截規則。`disable_udp_quic` 的目的是讓 LAN 用戶端的 YouTube 等 QUIC 流量降級到 TCP 以便代理，與核心節點通訊無關。
>
> 若 GSO 選項開啟後問題仍存在，建議查閱 [Mihomo Wiki Hysteria 設定](https://wiki.metacubex.one/config/proxies/hysteria/) 或 [Hysteria2 設定](https://wiki.metacubex.one/config/proxies/hysteria2/) 驗證節點欄位是否正確。
...
- **dns.enhanced-mode**: 根據 Fake-IP / Redir-Host 設定 → 影響 Mihomo 的 DNS 解析策略：
  - `fake-ip`: 所有 DNS 查詢返回 198.18.x.x 假 IP，規則基於網域名稱比對，效能最優
  - `redir-host`: DNS 在用戶端完成，規則基於真實 IP 比對，適合 BT/PT
- **tun.enable**: `en_mode_tun != 0` 時設為 `true` → Mihomo 建立 `utun` 虛擬網路卡接管流量
- **tun.stack**: `system`(系統協定棧)/`gvisor`(使用者態協定棧)/`mixed`(TCP system + UDP gvisor)
...
- **Fake-IP (非 TUN)**: 同上 + 額外比對 `fakeip_range`(198.18.0.1/16) 的路由
...
> **生效路徑**: 絕大多數流量控制選項不修改 YAML，而是影響 `set_firewall()` 產生的 iptables/nftables 規則鏈。
>
> **AI 行為指引**: 當使用者詢問流量路由問題時（如"TUN 和 TPROXY 有什麼區別"、"如何讓某裝置不走代理"、
> "旁路由/閘道器模式下如何設定"、"IPv6 流量如何控制"），AI 應結合本章節的防火牆規則詳解
...
> **注意**：如需按介面/使用者/DSCP 等維度精細繞過，請使用「外掛設定頁面底部 → 來源流量存取控制 (2.10)」。黑白名單裝置級繞過使用「外掛設定 → 黑白名單 (2.4)」。
...
- **實現細節**: 控制 OUTPUT 鏈規則是否產生。開啟時建立 `openclash_output` 鏈（fw4）或 OUTPUT 規則（fw3），將路由器自身出站流量重定向到 Clash。關閉時刪除 OUTPUT 鏈規則，路由器自身流量走原始路由表。

### disable_udp_quic — 停用 QUIC (Disable QUIC)
- **UCI 選項**: `openclash.@openclash[0].disable_udp_quic`
- **預設**: 1 (開啟)
- **效果**: 對 UDP 443 埠的流量執行 REJECT，阻止 YouTube 等使用 QUIC 協定傳輸 (降級到 TCP)
...
### skip_proxy_address — 繞過伺服器位址 (Skip Proxy Address)
- **UCI 選項**: `openclash.@openclash[0].skip_proxy_address`
- **預設**: 0 (關閉)
- **說明**: 繞過設定中伺服器位址的代理，防止重複代理 (代理巢狀)
- **實現細節**: 開啟後看門狗指令碼 `openclash_watchdog.sh` 中的 `skip_proxies_address()` 函式（每 30 個看門狗週期執行一次）解析 YAML 中所有代理節點（`proxies` 和 `proxy-providers`）的 `server` 位址，網域名稱透過 `openclash_debug_dns.lua` 呼叫 Mihomo 核心 API（`/dns/query`）解析為 IP 後，加入已存在的 `localnetwork` nft set（或 ipset）。由於 `openclash` 鏈首條規則為 `ip daddr @localnetwork counter return`，這些代理伺服器 IP 自動被跳過代理，防止代理巢狀。
...
- **自訂格式**: 空格分隔的埠號，如 `443 80` 或範圍 `20-443`
- **依賴**: 僅 Redir-Host 系列模式
- **實現細節**: 非 0 時在 `openclash` redirect 鏈的埠檢查規則中使用自訂埠列表（而非預設的允許所有埠）。格式 `{tcp, udp} th dport {80, 443, ...} counter redirect to proxy_port`。停用時規則不含埠限制，所有 TCP 都重定向。
...
- **可選值**:
  ...
  - `2` — 繞過海外 IP
- **說明**: 強烈推薦啟用「繞過中國大陸」。啟用後，會在 `fake-ip-filter` 新增 `rule-set:oc-cn-domain` 規則集 (舊版本為 GeoSite 資料庫中分類為 `CN` 的網域名稱)，且解析 IP 位於大陸 IP 段範圍內的流量將不進入核心，顯著降低核心效能開銷。旁路由模式下如果遇到大陸網域名稱無法存取可嘗試開啟"旁路由相容"選項
- **Mihomo 對應**: 透過 `dns.fake-ip-filter` 新增 `rule-set:oc-cn-domain` 規則集，使中國大陸網域名稱返回真實 IP 而非 Fake-IP；同時自動註冊對應的 `rule-providers` 條目指向 MetaCubeX geosite CN MRS 檔案
- **實現細節（雙重機制）**: 1) **YAML 層面**: `yml_change.sh` 修改 `dns.fake-ip-filter`——blacklist 模式（預設）追加 `rule-set:oc-cn-domain`，whitelist 模式移除 CN 相關過濾器，rule 模式前置 `RULE-SET,oc-cn-domain,real-ip`。效果：比對的中國大陸網域名稱返回真實 IP，繞過 Fake-IP 機制。2) **防火牆層面**: `set_firewall()` 使用 chnroute IP 列表構建 nftables set（`china_ip_route` 或者 `china_ip6_route`），在 redirect/TPROXY 鏈中比對國內真實 IP 直連 return。兩層面互為補充——YAML fake-ip-filter 確保大陸網域名稱獲得真實 IP，防火牆 nft set 比對這些真實 IP 使其跳過代理。

### intranet_allowed — 僅允許內網 (Only Intranet Allowed)
- **UCI 選項**: `openclash.@openclash[0].intranet_allowed`
- **預設**: 1 (開啟)
- **說明**: 開啟後控制面板和連線代理埠僅能從內網存取，不暴露到公網
- **Mihomo 對應**: `allow-lan: true` + `bind-address: "*"`
- **實現細節**: 雙重保護——1) YAML 層面：`yml_change.sh` 設定 `allow-lan: true` + `bind-address: "*"` 使核心監聽所有介面。2) 防火牆層面：建立 `openclash_wan_input` 鏈，比對來自 WAN 口（非 `@localnetwork` 源 IP）的流量，REJECT 全部 7 個服務埠（`$proxy_port`/`$tproxy_port`/`$cn_port`/`$http_port`/`$socks_port`/`$mixed_port`/`$dns_port`）。關閉時 `allow-lan: false`，核心僅監聽 `127.0.0.1`，同時不建立 `openclash_wan_input` 鏈。
...
- **可選值**: 系統中所有網路介面名
- **預設**: 0 (停用)
- **說明**: 指定 LAN 介面名稱，用於透過 `ip address show <介面>` 取得路由器 LAN IP 位址（供控制面板位址顯示、API 呼叫、除錯日誌等使用）。設為 0 則自動檢測

### local_network_pass — 本機 IPv4 網路繞過列表 (Local Network Pass)
- **UCI 選項**: `openclash.@openclash[0].local_network_pass`
- **儲存檔案**: `/etc/openclash/custom/openclash_custom_localnetwork_ipv4.list`
- **說明**: 目標位址為列表中 IP 的流量不經過核心
...
- **說明**: 列表中的網域名稱/IP 不受中國 IP 繞行選項影響，依賴 Dnsmasq。**預設已預置** `services.googleapis.cn`、`googleapis.cn`、`xn--ngstr-lra8j.com` 以解決 Google Play 下載問題
- **依賴**: `enable_redirect_dns != 2`
- **注意**: chnroute_pass 僅在 DNS 解析層面將網域名稱解析 IP 加入 `china_ip_route_pass` nft set 使其跳過繞行規則，但若上游 DNS 本身將這些網域名稱解析到國內 IP，加入 set 後仍會被 `china_ip_route` 規則誤判為國內 IP 而繞行。**僅靠 chnroute_pass 不足以解決 Google Play 下載問題**——必須同時從 DNS 解析（`nameserver-policy` 強制走境外 DNS）和規則比對（自訂規則走代理）兩方面入手，詳見錯誤速查表 §十四
...
> 2. `change_dnsmasq()` 修改系統 dnsmasq → LAN 用戶端 DNS 被劫持到 Clash
> 3. `openclash_custom_domain_dns.sh` 為自訂網域名稱設定獨立 DNS
>
> **AI 行為指引**: 當使用者詢問 DNS 劫持相關問題時（如"DNS 重定向模式選哪個"、"自訂上游 DNS 伺服器怎麼寫"、
> "Fake-IP 和 Redir-Host 的 DNS 行為有何不同"、"如何讓特定網域名稱不走 Fake-IP"），AI 應結合本文件的
> 「防火牆與 DNS 規則詳解」章節和 [Mihomo DNS 設定文件](https://wiki.metacubex.one/config/dns/)
...
### enable_redirect_dns — 本機 DNS 劫持 (Redirect Local DNS Setting)
...
- **說明**:
  ...
  - **防火牆重定向** (值2): 透過 iptables/nftables 將發往 53 埠的 UDP/TCP 流量 DNAT 到核心 DNS 埠。Fake-IP 模式下使用 LAN 存取控制必須選此項
- **Mihomo 對應**: DNS 監聽設定 `dns.listen: 0.0.0.0:7874`
- **實現詳解**:
  - **值1 (Dnsmasq)**: `change_dnsmasq()` 函式先備份 dnsmasq 原有設定到 `openclash.config.*`，然後設定 `dhcp.@dnsmasq[0].server=127.0.0.1#<dns_port>`，`noresolv=1`，`cachesize=0`。效果：所有 LAN 用戶端的 DNS 查詢 → dnsmasq → 轉發到 Clash DNS (7874) → Clash 根據 `enhanced-mode` 處理。
  - **值2 (防火牆)**: 建立 `openclash_dns_redirect` nftables 鏈，對目標埠 53 的 UDP/TCP 流量 DNAT 到 `dns_port`。同時保留 dnsmasq 處理本機 DNS 快取。此模式允許 `lan_ac_*` 存取控制（需要 Fake-IP 模式）。
  - **恢復**: `revert_dnsmasq()` 還原所有原始 dnsmasq 設定（servers、noresolv、resolvfile、cachesize）。
...
- **功能**: 停止 OpenClash 後 DNS 異常時使用。恢復 Dnsmasq 預設設定:
  1. 設定 `noresolv=0`, `localuse=1`
  2. 恢復 `resolvfile` 為有效的 DNS 設定檔案
  3. 若無有效設定則建立 `/tmp/resolv.conf.d/resolv.conf.auto` (114.114.114.114, 8.8.8.8)
  4. 重啟 dnsmasq
...
- **預設**: 0
- **說明**: 為自訂網域名稱列表指定專用 DNS 伺服器，完全獨立於核心 DNS 查詢

### custom_domain_dns_server — 指定伺服器 (Specify DNS Server)
- **UCI 選項**: `openclash.@openclash[0].custom_domain_dns_server`
- **預設**: `114.114.114.114`
- **格式**: `IP位址` 或 `IP位址#埠` (如 `127.0.0.1#5300`)

### custom_domain_dns — 自訂網域名稱列表 (Custom Domain DNS)
- **儲存檔案**: `/etc/openclash/custom/openclash_custom_domain_dns.list`
- **格式**: 每行一個網域名稱
- **說明**: 列表中的網域名稱不返回 Fake-IP，使用指定的上游 DNS 伺服器解析

---

## 2.4 黑白名單標籤頁 (Black&White / lan_ac)

> **生效路徑**: 存取控制完全在防火牆層面實現，不修改 YAML。
>
> **AI 行為指引**: 當使用者詢問存取控制問題時（如"如何讓某個裝置不走代理"、"如何讓內網某裝置全域性代理"、
> "代理黑名單和白名單的區別"），AI 應結合本章節的防火牆規則詳解
> （特別是「各選項對防火牆規則的具體影響」表格）告知使用者各選項組合的效果。
> 涉及黑白名單比對邏輯時，查閱 [OpenClash 原始碼](https://github.com/vernesong/OpenClash/tree/dev) 中
...
> **注意**：如需按介面/使用者/DSCP 等維度精細繞過，請使用「外掛設定頁面底部 → 來源流量存取控制 (2.10)」。
> **依賴**: `enable_redirect_dns=2`（防火牆重定向模式）僅在 **Fake-IP 模式**下強制要求——因為 Fake-IP 返回虛擬 IP，用戶端不知道真實目標，
> 必須透過防火牆重定向 DNS 才能實現基於真實目標的存取控制。Redir-Host 模式下此依賴為可選（LuCI UI 中同樣要求，但底層機制不同）。

### lan_ac_mode — 區域網存取控制模式 (LAN Access Control Mode)
...
- **實現細節**: 系統使用 `ebtables` 或 `nft` 在二層網橋層面比對 MAC 位址，使用 `nftables` 在三層比對 IP。黑白名單決定規則的 return 行為取反（黑名單=比對到return直連，白名單=不比對則return直連）。

### lan_ac_black_ips — 不走代理的區域網裝置 IP (LAN Bypassed Host List)
- **UCI 選項**: `openclash.@openclash[0].lan_ac_black_ips` (DynamicList)
- **格式**: IP 位址或 CIDR 網段
- **依賴**: `lan_ac_mode=0`
- **實現細節**: 產生 nftables set `openclash_lan_black_ip` / `openclash_lan_black_ip6`，在 `openclash` redirect 鏈中插入 `ip saddr @openclash_lan_black_ip counter return` 跳過規則。

### lan_ac_black_macs — 不走代理的區域網裝置 Mac (LAN Bypassed Mac List)
- **UCI 選項**: `openclash.@openclash[0].lan_ac_black_macs` (DynamicList)
- **格式**: MAC 位址
- **實現細節**: 透過 `ebtables`（fw3）或 `nft add rule bridge`（fw4）在 br-lan 網橋上比對源 MAC，比對到的流量不進入 Clash 代理鏈。
...
- **說明**: Fake-IP 模式下僅支援純 IP 請求，網域名稱請求需先設定 Fake-IP-Filter
...
> **生效路徑**: `openclash_streaming_unlock.lua` 指令碼在核心啟動後被守護程式呼叫，
...
> 瞭解 GEOSITE/RULE-SET 等規則型別，並結合 OpenClash 的流媒體增強設定告知使用者具體設定步驟。
> 涉及流媒體解鎖檢測邏輯時，查閱 [OpenClash 原始碼](https://github.com/vernesong/OpenClash/tree/dev) 中
> `openclash_streaming_unlock.lua` 指令碼。
> 注意：`openclash_streaming_unlock.lua` 僅負責已設定服務的節點測試和自動切換，不負責 Smart 策略。

所有子選項依賴 `stream_auto_select=1`。每個流媒體服務有一組獨立設定：
...
每個服務設定項：
- **策略組篩選 (Group Filter)**: `stream_auto_select_group_key_<service>` — 比對策略組的正規表示式
...
- **實現詳解**: `openclash_streaming_unlock.lua` 是一個獨立 Lua 指令碼，被 `/etc/init.d/openclash` 以 `procd` 服務形式啟動（與核心並列）。工作流程：
  ...
  2. 對每個啟用的流媒體服務，嘗試用各節點連線服務網域名稱（如 `netflix.com`）
  3. 檢查 HTTP 回應碼或頁面內容判斷是否解鎖（如 Netflix 返回 200 且不含地區限制頁面則解鎖）
  4. 找到能解鎖的節點後，透過 Mihomo API `PUT /proxies/{group}` 自動切換策略組到該節點
  5. 定期重新測試（間隔可設定），節點失效時自動切換
...
| 管理頁面公網網域名稱 (Public Dashboard Address) | `dashboard_forward_domain` | 空 | 用於公網存取面板 |
| 管理頁面對映埠 (Public Dashboard Port) | `dashboard_forward_port` | 空 | |
| 管理頁面公網 SSL 存取 (Public Dashboard SSL enabled) | `dashboard_forward_ssl` | 0 | |

儀表盤版本管理透過 `action_switch_dashboard` → `openclash_download_dashboard.sh` 自動下載切換。
- **實現細節**: `yml_change.sh` 將 `cn_port`、`dashboard_password` 寫入 YAML → Mihomo 核心啟動 HTTP API。`openclash_download_dashboard.sh` 從 GitHub Releases 下載 Dashboard 靜態檔案 (yacd/metacubexd/zashboard)，解壓到 `/usr/share/openclash/ui`。前端 `status.htm` 中的 JS 根據以下邏輯構造儀表盤 URL：1) 若當前瀏覽器 hostname 與路由器 LAN IP 相同 → `http://<lan_ip>:<cn_port>/ui/<dashboard>/`；2) 若設定了 `dashboard_forward_domain` + `dashboard_forward_port`（公網存取）→ 協定由 `dashboard_forward_ssl` 決定（0=http, 1=https），位址為 `<protocol>://<domain>:<port>/ui/<dashboard>/`；3) 其他情況 → 取當前頁面協定 + LAN IP + cn_port。四個儀表盤的子路徑分別為 `/ui/dashboard/`、`/ui/yacd/`、`/ui/metacubexd/`、`/ui/zashboard/`。

---

## 2.7 IPv6 設定標籤頁 (ipv6)

> **注意：** 不建議為路由器開啟 IPv6 及相關服務。IPv6 方案僅適用於**主路由撥號環境**（需電信業者支援 IPv6-PD 字首下發），旁路由環境不適用
> **生效路徑**: IPv6 選項透過 `yml_change.sh` 寫入 YAML（`ipv6`、`dns.ipv6`、`dns.fake-ip-range6`），
> 同時 `set_firewall()` 產生獨立的 IPv6 防火牆規則鏈（`openclash_v6`、`openclash_mangle_v6` 等）。
> IPv6 使用單獨的 TProxy/Redirect/TUN 規則鏈，與 IPv4 互不影響。
>
> **IPv6 DNS 核心最佳實踐**：DNS 解析請求（包括 AAAA 記錄查詢）可以透過 IPv4 鏈路傳送，無需 IPv6 DNS 伺服器。推薦策略：① LAN 介面 DHCP 伺服器中**不分配 IPv6 DNS**，強制裝置用路由器 IPv4 位址做 DNS 解析；② 取消 `過濾 IPv6 AAAA 記錄`（Dnsmasq 高階設定）；③ 開啟 OpenClash 的「允許 IPv6 型別 DNS 解析」選項。效果：DNS 走 IPv4 通道查詢（經過 OpenClash 分流），流量走 IPv6 通道傳輸——既防止 IPv6 DNS 搶答導致的洩露，又保證 IPv6 站點可存取
...
| IPv6 Fake-IP 範圍 (Fake-IP Range) | `fakeip_range6` | 停用 | 僅 Fake-IP 模式。對應 `dns.fake-ip-range6` |
| 實驗性：繞過指定區域 IPv6 (China IPv6 Route) | `china_ip6_route` | 0 | 0=關閉, 1=繞過大陸, 2=繞過海外 |
| 本機 IPv6 繞過位址 (Local IPv6 Network Bypassed List) | `local_network6_pass` | — | 檔案: `/etc/openclash/custom/openclash_custom_localnetwork_ipv6.list` |
| 繞過指定區域 IPv6 黑名單 (Chnroute6 Bypassed List) | `chnroute6_pass` | — | 檔案: `/etc/openclash/custom/openclash_custom_chnroute6_pass.list`。將列表中網域名稱/IP 加入 `china_ip6_route_pass` nft set，不受 IPv6 繞行選項影響。依賴: `ipv6_enable=1` + `enable_redirect_dns=1` |
...
| 資料型別 | 啟用 UCI | 更新指令碼 | 更新星期 UCI | 更新時間 UCI | 自訂 URL UCI |
|----------|----------|----------|-------------|-------------|---------------|
...
**共享設定項**：`*_update_week_time` (周幾): `*`=每天, `1`=週一, `2`=週二, …, `0`=週日; `*_update_day_time` (小時): `0`-`23`; `*_custom_url` (自訂下載連結，留空使用預設)

**Mihomo 對應**: `geox-url` 中的各欄位 + `geo-auto-update` + `geo-update-interval`
- **實現細節**:
  - **Cron 觸發**: `add_cron()` 在 `openclash_geo.sh` 中為每種 GEO 型別註冊 cron 任務
  - **下載流程**: `openclash_geo.sh` 使用自訂 URL（`*_custom_url`）或預設位址下載，儲存到 `/etc/openclash/` 目錄
  - **Mihomo 使用**: MMDB 用於 `GEOIP` 規則比對（IP→國家），Dat 用於 `GEOSITE` 規則比對（網域名稱→類別），ASN 用於 Smart 策略
...
| 大陸 IP 段更新 URL (Custom Chnroute Lists URL) | `chnr_custom_url` | `https://ispip.clang.cn/all_cn.txt` | 中國 IPv4 CIDR 列表下載連結 |
| 大陸 IPv6 段更新 URL (Custom Chnroute6 Lists URL) | `chnr6_custom_url` | `https://ispip.clang.cn/all_cn_ipv6.txt` | 中國 IPv6 CIDR 列表下載連結 |
...
此標籤頁內嵌「檢查更新」面板（update 模板的 version_tab 模式），以 **CDN 位址列表** 為核心，提供核心/外掛版本選擇、下載和更新操作。頁面載入時透過 `/update`（action_update，本機設定與已裝版本）、`/version_history`（版本歷史）、`/addr_info`（CDN 位址列表）API 動態填充。遠端最新版本（corelv/oplv，status 頁紅點用）由獨立 `/last_version` 端點提供，本面板不使用。
...
| **編譯版本 (Compiled Version)** | `core_version` | Select | `0`(未設定) | 選擇與 CPU 比對的編譯版本：`linux-amd64-v1/v2/v3`(x86-64)、`linux-arm64`(armv8)、`linux-armv7`、`linux-mips64` 等 ~18 種架構。**未選擇（0/空）時核心無法下載**，點選會提示 "No Compiled Version Selected" |
| **更新分支 (Release Branch)** | `release_branch` | Select | `master` | `master`(穩定版) / `dev`(開發版)，決定外掛/核心的下載分支 |
| **Smart 核心 (Smart Core)** | `smart_enable` | Select | `0` | `0`=停用(使用 Meta 核心) / `1`=啟用(使用 Smart 核心)，決定核心下載的 meta/smart 子路徑 |
...
| **Installed（核心）** | 已安裝核心版本 | 只讀，執行 `clash_meta -v` 取得（`coremetacv`） |
...
**CDN 位址列表**（Address / Latency / Plugin / Core 四列）:

- 預設 CDN：`raw.githubusercontent.com`（RAW 直連）、`fastly.jsdelivr.net`、`testingcf.jsdelivr.net`、`cdn.jsdelivr.net`；也可在底部「Custom Your CDN URL」新增自訂 CDN（如 `https://ghfast.top/`，新增後只拉取新增 CDN）
- 每行 Latency 列顯示測速延遲（`xxx ms`，按快/中/慢著色）或「Access Timed Out / Access Denied」；Plugin/Core 列顯示該 CDN 取得到的外掛/核心版本號
...
| 操作 | 功能 |
|------|------|
| **備份 (Backup File)** | 先選備份範圍下拉（Backup File 完整 / Backup Exclude Cores 排除核心 / Backup Core 僅核心 / Backup Config 僅設定 / Backup Rule Provider 僅規則提供者 / Backup Proxy Provider 僅代理提供者），再點按鈕下載備份 |
| **還原預設 (Restore Default)** | 恢復 OpenClash 為預設出廠設定（確認後跳回設定頁） |
| **刪除核心 (Remove Core)** | 刪除所有核心二進位制檔案（紅色危險按鈕） |

- **實現細節**:
  - 所有設定修改（編譯版本/分支/Smart）透過 `/save_corever_branch` API 即時儲存到 UCI。
  ...
  - **下載 URL 結構**：`package`/`core` 是倉庫**分支**（非目錄）。Latest（無歷史 sha）：`package/{branch}/luci-app-openclash_{ver}_all.ipk`、`core/{branch}/{meta|smart}/clash-{arch}.tar.gz`（把分支名作為 ref 段）；選擇歷史版本（帶 sha，為對應分支的 CI commit）：`{sha}/{branch}/luci-app-openclash_{ver}_all.ipk`、`{sha}/{branch}/{meta|smart}/clash-{arch}.tar.gz`。jsdelivr CDN 字首為 `gh/vernesong/OpenClash@{ref}/...`，自訂 CDN 字首直接拼接 raw URL。
  - **下載失敗不會觸發 OpenClash 重啟**（`openclash_core.sh` 僅在核心真正更新成功後才置重啟標誌）。
  - 外掛更新透過 ubus 後臺安裝以避免 Web 介面斷連。

### 開發者選項 (Developer Settings / developer)
- **自訂防火牆規則** (`firewall_custom`): 在 LuCI 的「開發者設定」標籤頁中直接編輯的文字框，內容儲存到 `/etc/openclash/custom/openclash_custom_firewall_rules.sh`。該指令碼**不需要定義任何函式**——它是一個指令式 Shell 指令碼，在所有 OpenClash 內建防火牆規則新增完畢後被直接執行（`chmod +x` 後執行）。可以在指令碼中直接寫 `iptables -I ...` 或 `nft add rule ...` 指令來追加自訂防火牆規則。
- **實現細節**: `set_firewall()` 函式在所有內建的 REDIRECT/TPROXY/TUN/IPv6/存取控制規則建立完畢後，檢查此檔案是否存在，若存在則 `chmod +x` 並執行。由於它在所有內建規則之後執行，自訂規則可以引用 OpenClash 已建立的 nftables 鏈和 set。每次 OpenClash 啟動或防火牆過載時都會重新執行此指令碼。
...
| 工具 | 功能 | 觸發方式 | 後端路由 |
|------|------|---------|----------|
| **連線測試 (Connection Test)** | 測試指定網域名稱是否可達 | 輸入網域名稱 + 點選「Click to Test」(點選測試) 按鈕 | `/diag_connection` |
| **DNS 測試 (DNS Test)** | 測試 DNS 解析結果 | 輸入網域名稱 + 點選「Click to Test」(點選測試) 按鈕 | `/diag_dns` |

**連線測試實現細節**: 前端先嚐試 `Image` 載入 `https://{domain}/favicon.ico` 作為快速預檢，若失敗則回退到後端 `/diag_connection` 呼叫。

### oixCloud (oixcloud)
- 第三方雲服務，需賬號密碼登入
- `oix_email` / `oix_passwd` → `oix_login` 取得 token
- `oix_checkin` — 自動簽到 (需 token)
- 登入後自動取得 Oix 專屬核心和訂閱

---

## 2.10 來源流量存取控制 (Source Traffic Bypass / lan_ac_traffic)

> **頁面位置**：外掛設定頁面底部（不屬於任何標籤頁，以獨立的 TypedSection 形式存在）
> **生效路徑**：透過 `init.d/openclash` 的 `firewall_lan_ac_traffic()` 函式在代理鏈 **position 0** 插入規則，
> 優先順序高於所有其他代理規則。設定透過 `config_foreach firewall_lan_ac_traffic "lan_ac_traffic"` 遍歷執行。
>
> **AI 行為指引**：當使用者詢問「如何讓某個介面（如 WireGuard/Docker 網橋）的流量完全繞過核心」、
> 「如何按使用者 UID 繞過代理」、「如何精細控制特定來源流量」時，AI 應告知使用者使用此功能，
> 並結合下方欄位表和防火牆邏輯給出具體設定方案。涉及底層實現時查閱
...
支援按七維度組合比對流量，比對後執行 target 動作：

| 欄位 | UCI Key | 型別 | 說明 |
|------|---------|------|------|
| 備註 | `comment` | Value | 規則說明 |
| 啟用 | `enabled` | Flag | 預設 1 |
| 內部位址 | `src_ip` | Value | IP/CIDR/`localnetwork`（比對區域網路位址集 @localnetwork） |
| 內部埠 | `src_port` | Value | 埠或範圍，如 `5000` 或 `1234-2345` |
| 協定 | `proto` | ListValue | `both`(預設)/`tcp`/`udp` |
| 位址族 | `family` | ListValue | `both`(預設)/`ipv4`/`ipv6` |
| 介面 | `interface` | ListValue | 網路介面名（如 `eth1`、`wg0`、`docker0`），比對從該介面進入的流量 |
| 使用者 | `user` | ListValue | Linux UID，比對該使用者程式發出的流量（僅 OUTPUT 鏈生效） |
| DSCP | `dscp` | Value | 0-63，比對 IP 頭 DSCP 標記值 |
...
config_foreach firewall_lan_ac_traffic "lan_ac_traffic"
  ...
  → 構建 nftables/iptables 比對條件
  → 按執行模式 + 協定 + 位址族插入對應鏈的 position 0
```
...
- **`src_ip=localnetwork`**：特殊值，nftables 端展開為 `ip saddr @localnetwork`（比對整個區域網路位址集），iptables 端使用 `-m set --match-set localnetwork src`
- **介面比對**：nftables 用 `iifname "介面名"`，iptables 用 `-i 介面名`
- **使用者比對**：nftables 用 `meta skuid UID`（僅 OUTPUT 鏈生效），iptables 用 `-m owner --uid-owner UID`
- **DSCP 比對**：nftables 用 `ip dscp 值`，iptables 需 `dscp` 模組（不可用時跳過並輸出警告：`iptables DSCP module not available`）
- **Fake-IP 排除**：所有規則自動排除 Fake-IP 位址段 `ip daddr != {198.18.0.0/16}`，避免影響 Fake-IP 內部對映
...
| 某使用者程式流量完全丟棄 | `user=65534`, `target=DROP` |
| 區域網路所有 UDP 流量直連 | `src_ip=localnetwork`, `proto=udp`, `target=RETURN` |
...
> 此頁面用於覆寫訂閱設定中的特定欄位，設定後透過 openclash.sh 指令碼注入到產生的 YAML 中
...
```
 ...
 └─ 自訂 DNS servers              └─ 自訂規則注入
```

**關鍵機制**: `yml_change.sh` 以 YAML 深度合併 + 覆蓋的方式修改設定，覆寫優先順序高於訂閱原始值。
`yml_rules_change.sh` 使用 Ruby YAML 庫操作策略組、規則、URL-Test 引數和規則提供者位址。
...
### urltest_address_mod — 測速（連通性）位址修改 (URL-Test Address Modify)
...
### github_address_mod — Github 位址修改 (Github Address Proxy)
...
- **預設**: 多個 jsdelivr CDN 位址（testingcf / fastly 等）
- **實現細節**: `yml_rules_change.sh` 用 Ruby 正則 `/raw\.githubusercontent\.com/` 比對所有 rule-providers 和 proxy-providers 的 `url` 欄位，將網域名稱替換為 CDN 位址。解決中國大陸無法存取 GitHub 的問題。
- **已知限制**: `github_address_mod` 僅對 rule-providers 和 proxy-providers 的 URL 生效。`openclash_download_dashboard.sh`（Dashboard 下載）和 `openclash_geo.sh`（GEO 更新）**不使用此變數**，這些指令碼的下載 URL 為硬編碼的 GitHub 直連位址。如需對 Dashboard/GEO 下載使用 CDN，可透過覆寫模組的 `[General]` 段設定 `DOWNLOAD_FILE` 或使用自訂規則使相關網域名稱直連。

### log_level — 日誌等級 (Log Level)
- **UCI**: `openclash.@config_overwrite[0].log_level`
- **可選值**: `0`(停用) / `info` / `warning` / `error` / `debug` / `silent`
- **Mihomo 對應**: `log-level`
- 0 表示不覆寫，使用訂閱原有設定
...
- **埠實現細節**: `yml_change.sh` 將所有埠寫入 YAML 對應欄位。Mihomo 核心啟動時在這些埠上建立監聽器，接受來自 iptables/nftables 重定向的流量或用戶端直連的代理請求。修改後需重啟核心。

## 3.2 DNS 設定標籤頁 (DNS Settings / dns)

> **生效路徑**: DNS 覆寫透過 `yml_change.sh` 的 `yml_dns_custom()` 函式處理，
> 構建完整的 `dns:` YAML 段併合併到執行設定。
>
> **AI 行為指引**: 當使用者詢問 DNS 設定問題時（如"如何設定 DoH/DoT"、"nameserver-policy 怎麼寫"、"hosts 格式是什麼"、
> "fallback-filter 各欄位含義"等），AI 應查閱 [Mihomo DNS 設定文件](https://wiki.metacubex.one/config/dns/)
...
> 然後**結合 OpenClash 覆寫模組的操作方式**告知使用者如何設定，而非僅給出文件連結。

### enable_custom_dns — 自訂上游 DNS 伺服器 (Custom DNS Setting)
- **UCI**: `openclash.@config_overwrite[0].enable_custom_dns`
- **預設**: 0
- **說明**: 開啟後將透過 TypedSection `dns_servers` 中的設定覆寫 YAML 的 `dns` 段
- **最佳實踐**: 在 Fake-IP 模式下推薦以下設定策略：① Nameserver 僅負責直連類網域名稱的解析（使用電信業者 DNS 或國內 DoH 如 AliDNS/DNSPod）；② **取消所有 Fallback 伺服器**——Fake-IP 模式下若無 Fallback，非直連網域名稱的解析請求將交由遠端（代理節點側）完成，解析結果與實際出站鏈路一致，可獲得更一致的 CDN 命中並防止 DNS 洩露；③ 若出站側解析不可用（罕見），可啟用 Fallback 作為兜底並同時開啟「遵循規則」功能。**不建議套娃其他 DNS 外掛**（如 MosDNS/SmartDNS/AdGuardHome），多外掛疊加會引入快取一致性問題、增加內網解析延遲，且破壞 Mihomo 向用戶端傳遞的 TTL 值
...
- **實現細節**: 寫入 YAML `dns.respect-rules: true`。Mihomo 核心的 DNS 解析器發出的連線將經過 `rules` 規則引擎比對——意味著 DNS 查詢本身也會被代理（透過比對的代理節點發出），防止 DNS 洩露。需要配合 `proxy-server-nameserver` 防止雞生蛋問題。

### append_wan_dns — 附加上游 DNS (Append WAN DNS)
- **UCI**: `openclash.@config_overwrite[0].append_wan_dns`
- **預設**: 1
- **說明**: 將 WAN 口自動分配的電信業者 DNS 和閘道器 IP 追加到 nameserver 列表。**主路由撥號環境推薦啟用**：電信業者 DNS 對直連類網域名稱的解析延遲通常最低（1-2ms），CDN 命中更接近實際鏈路，省去手動設定的麻煩。若使用第三方加密 DNS（如 DoH/DoT），則需停用此項並在 NameServer 中手動新增伺服器
- **實現細節**: `sys_dns_append()` 呼叫 `openclash_get_network.lua` 取得 WAN 口的 DNS 和閘道器位址，追加到 `/tmp/yaml_config.namedns.yaml`，後續被合併到 YAML `dns.nameserver`。支援 dhcp:// 協定直接從 DHCP 介面取得 DNS。
...
- **僅**: Fake-IP 模式顯示
- **實現細節**: 寫入 YAML `dns.fake-ip-range`。Mihomo 在 Fake-IP 模式下，將 DNS 查詢的網域名稱對映到此 CIDR 段中的虛擬 IP。應用連線到虛擬 IP 時核心透過路由表將流量導向 Clash，Clash 根據對映表還原真實網域名稱後進行規則比對。

### store_fakeip — 持久化 Fake-IP (Store Fake-IP)
- **UCI**: `openclash.@config_overwrite[0].store_fakeip`
- **預設**: 1
- **Mihomo 對應**: `profile.store-fake-ip`
- **說明**: 快取 Fake-IP DNS 解析記錄到檔案，啟動後加速回應
- **實現細節**: 寫入 YAML `profile.store-fake-ip: true`。Mihomo 將網域名稱→Fake-IP 對映持久化到 `cache.db` 檔案，重啟後恢復對映，避免重啟後所有網域名稱需要重新解析。

### custom_fallback_filter — 自訂 Fallback-Filter (Custom Fallback Filter)
- **UCI**: `openclash.@config_overwrite[0].custom_fallback_filter`
- **預設**: 0
- **說明**: 設定 DNS 防汙染回退過濾器
- **設定檔案**: `/etc/openclash/custom/openclash_custom_fallback_filter.yaml`
- **Mihomo 對應**: `dns.fallback-filter` 段

> Fallback-Filter 格式範例:
> ```yaml
> geoip: true
> geoip-code: CN
> geosite:
...
### custom_fakeip_filter — 自訂 Fake-IP-Filter (Custom Fake-IP Filter)
...
- **預設**: `blacklist`
- **說明**:
  - `blacklist`: 比對成功的網域名稱不返回 Fake-IP (黑名單)
  - `whitelist`: 只有比對成功的網域名稱返回 Fake-IP (白名單)
  - `rule`: 規則模式，支援 GEOSITE、RuleSet、DOMAIN* 等語法
- **Mihomo 對應**: `dns.fake-ip-filter-mode`

### 網域名稱過濾檔案 (custom_fake_filter)
- **檔案**: `/etc/openclash/custom/openclash_custom_fake_filter.list`
- **格式**: 每行一個網域名稱萬用字元，如 `*.lan`, `+.example.com`

### custom_name_policy — 自訂 Nameserver-Policy (Custom Name Policy)
...
- **格式**: 每行 `網域名稱=DNS伺服器組` 或使用 geosite/rule-set

### custom_proxy_server_policy — 自訂 Proxy-Server-Nameserver-Policy (Custom Proxy Server Policy)
...
- **說明**: 僅用於解析代理節點網域名稱的 DNS 策略

### custom_host — 自訂 Hosts (Custom Hosts)
...
> [Mihomo DNS 型別文件](https://wiki.metacubex.one/config/dns/type/) 瞭解每種 DNS 協定的使用方法和引數，
> 涉及 OpenClash 側實現時查閱 [OpenClash 原始碼](https://github.com/vernesong/OpenClash/tree/dev) 中
> `yml_change.sh` 的 DNS 相關邏輯，然後告知使用者具體的設定寫法。
...
| 停用 IPv6 | `disable_ipv6` | 丟棄 AAAA 記錄 |
...
> "find-process-mode 各模式的含義"、"sniffer 如何自訂"、"geodata-loader 選哪個"），
> AI 應查閱 [Mihomo 全域性設定文件](https://wiki.metacubex.one/config/general/) 和
...
- **說明**: 同時使用 DNS 解析的所有 IP 位址發起連線，使用最先成功的連線
- **實現細節**: `yml_change.sh` 寫入 YAML `tcp-concurrent: true`。Mihomo 對每個目標網域名稱解析出所有 IP 後，同時向所有 IP 發起 TCP 連線，使用第一個 TCP 握手成功的連線，丟棄其餘。可大幅降低首次連線延遲，但會增加併發連線數。
...
- **實現細節**: 寫入 YAML `unified-delay: true`。Mihomo 在 URL-Test 延遲測量時計算 RTT（Round-Trip Time），而非簡單的 TCP 握手時間 + HTTP 回應時間。這樣 Shadowsocks、Trojan、VMess 等不同協定的節點延遲可公平比較。

### find_process_mode — 啟用程式規則 (Find Process Mode)
- **UCI**: `openclash.@config_overwrite[0].find_process_mode`
- **可選值**: `0`(停用) / `off` / `always` / `strict`
- **預設**: 0
- **Mihomo 對應**: `find-process-mode`
- **說明**: 依賴 `kmod-inet-diag` 核心模組。路由器上推薦 `off` 以提升效能
- **實現細節**: 寫入 YAML `find-process-mode`。控制 Mihomo 是否透過 Netlink INET_DIAG 比對每個連線的發起程式名（用於 PROCESS-NAME 規則）。路由器上設為 `off` 可避免核心模組依賴和效能開銷。

### enable_meta_sniffer — 啟用流量（網域名稱）探測 (Enable Sniffer)
- **UCI**: `openclash.@config_overwrite[0].enable_meta_sniffer`
- **預設**: 1
- **Mihomo 對應**: `sniffer.enable: true`
- **說明**: 防止網域名稱代理和 DNS 劫持失敗。透過嗅探 TLS/HTTP/QUIC 握手取得真實目標網域名稱
- **實現細節**: `yml_change.sh` 寫入完整的 `sniffer:` YAML 段：
  - `sniff.TLS.ports: [443, 8443]` — 解析 TLS ClientHello 中的 SNI 欄位取得網域名稱
  - `sniff.HTTP.ports: [80, 8080-8880]` — 解析 HTTP Host 頭取得網域名稱
  - `sniff.QUIC.ports: [443]` — 解析 QUIC Initial 包中的 SNI
  - `force-dns-mapping: true` (僅 Redir-Host) — 對 DNS 解析過的 IP 強制嗅探
  - `override-destination: true` — 用嗅探到的網域名稱覆蓋連線目標，確保規則基於網域名稱比對
...
- **說明**: 對所有未取得到網域名稱的流量進行強制嗅探（如直接 IP 連線）

### enable_meta_sniffer_custom — 自訂流量探測（嗅探）設定 (Custom Sniffer Settings)
- **UCI**: `openclash.@config_overwrite[0].enable_meta_sniffer_custom`
- **預設**: 0
- **說明**: 啟用後將使用下方文字框中的自訂 sniffer YAML 設定替代預設嗅探設定

### sniffer_custom — 自訂 Sniffer 文字框 (Sniffer Custom)
- **UCI**: `openclash.@config_overwrite[0].sniffer_custom`
- **儲存**: `/etc/openclash/custom/openclash_custom_sniffer.yaml`
- **說明**: 多行 YAML 文字框，可自訂完整的 `sniffer:` 設定段。僅在 `enable_meta_sniffer_custom=1` 時生效

### geodata_loader — Geodata 資料載入方式 (Geodata Loader)
- **UCI**: `openclash.@config_overwrite[0].geodata_loader`
- **可選值**: `0`(停用) / `memconservative` / `standard`
- **預設**: `memconservative`
...
- **說明**: 使用 Dat 格式替換 MMDB 格式 GeoIP 檔案。Dat 檔案較大需單獨下載，可透過「GEO 資料庫訂閱」頁面取得

### global_ua — 全域性 User-Agent (Global UA)
- **UCI**: `openclash.@config_overwrite[0].global_ua`
- **預設**: 0 (停用，使用系統預設 `clash.meta`)
- **Mihomo 對應**: `global-ua`
...
> `yml_change.sh` 將 Smart 訓練資料收集設定寫入 YAML（`profile.smart-collector-size`），
...
> "prefer-asn 是什麼"、"sample-rate 怎麼設定"、"LGBM 模型如何自訂下載"），AI 應：
...
>    ① 取得目標 IP/網域名稱的 ASN 資訊 → ② 優先檢查使用者手動選擇的節點 → ③ 查持久化快取
...
>    瞭解並告知使用者以下資訊：
>    - **資料來源**：`component/smart/lightgbm/collector.go` — 開啟 `smart_collect` 後核心會在 `/etc/openclash/` 下產生 CSV 訓練資料檔案（含延遲、抖動、丟包率等特徵；特徵工程見 `transform.go`）
>    - **LightGBM 版本**：查閱 `go.mod` 中 `vernesong/leaves` 依賴確認支援的 LightGBM 版本
>    - **環境搭建**：在 PC/伺服器上安裝對應版本的 LightGBM Python 包，準備訓練環境
>    - **訓練指令碼**：參考 `component/smart/lightgbm/` 中的特徵處理與模型結構，為使用者建立可執行的範例訓練指令碼（讀取 CSV → 特徵變換 → 訓練 LightGBM → 匯出 Model.bin）
>    - **模型部署**：訓練完成後將 `Model.bin` 上傳到可存取的 URL，透過 `lgbm_custom_url` 指定；或替換 `/etc/openclash/Model.bin`；模型載入與推理見 `lightgbm.go` 中的 `WeightModel`
>    - **日常使用**：大多數使用者無需自行訓練，開啟 `lgbm_auto_update` 即可自動下載預訓練模型
> **關鍵提醒**：Smart 策略使用 LightGBM 模型進行節點品質預測，需要在設定檔案中將策略組型別設為 `smart`
...
- **說明**: `<1` 降低優先順序，`>1` 提高優先順序，預設權重為 1。支援正則和字串比對策略組名稱
...
- **實現細節**: `yml_change.sh` 設定 YAML 中的模型下載 URL 和更新間隔。`openclash_lgbm.sh` 定期下載訓練好的 LightGBM 模型檔案到 `/etc/openclash/Model.bin`（小快閃記憶體模式下為 `/tmp/etc/openclash/Model.bin`）。Mihomo Smart 模組載入模型後，根據節點歷史延遲、抖動、丟包率等特徵預測最優節點。
...
- **預設**: 100 (MB)
- **Mihomo YAML 對映**: `profile.smart-collector-size: <size>` (全域性設定)
- **依賴**: `smart_collect=1`
...
### lgbm_custom_url — 自訂模型下載連結 (LGBM Custom URL)
...
> **LuCI 路徑**: 服務 → OpenClash → 設定管理 → 節點管理 → 編輯按鈕 (groups-config)
> **注意**: groups-config 不是主選單頁面，而是透過「設定管理 → 節點 & 策略組管理」頁面中的編輯按鈕載入進入的隱藏子頁面（controller 中註冊為 `nil` 顯示名）。
> **UCI Section**: `openclash.groups_config` (多條，每條對應一個策略組)
> 以下為 `type=smart` 策略組獨有的設定選項，用於**覆蓋**全域性 Smart 設定中的對應值。
...
| **策略優先順序** (Policy Priority) | `policy_priority` | *(空)* | `proxy-groups[].policy-priority: "<pattern>"` | 此策略組內節點的權重優先順序，格式同全域性 `smart_policy_priority`（如 `Premium:0.9;SG:1.3`）。支援正則比對節點名稱 |
...
> **注意**: Per-group 的 `tolerance` 欄位在 LuCI 介面中可設定，但實際 YAML 產生指令碼（`yml_rules_change.sh`）在 smart auto switch 處理中統一使用全域性 `smart_tolerance` 值應用到**所有** smart 型別策略組。如需對不同策略組設定不同 tolerance，需透過覆寫模組的 `[YAML]` 段手動指定。
>
> **Per-group Smart 設定的生效方式**: 這些欄位直接寫入策略組的 YAML 設定中（如 `proxy-groups[0].uselightgbm: true`），由 Mihomo Smart 核心在執行時讀取。它們與全域性 Smart 設定（覆寫設定 → Smart 設定）的關係是：**per-group 設定覆蓋全域性設定，但僅影響該策略組**。
>
...
> 此標籤頁用於管理自訂 Clash/Mihomo 路由規則。Mihomo 支援多種規則型別，
...
- **預設**: 0
- **說明**: 開啟後向設定追加 PROCESS-NAME 和 DST-PORT 規則，僅允許比對規則的流量走代理，其餘流量（如 BT/P2P）直連

### enable_custom_clash_rules — 自訂規則 (Custom Clash Rules)
- **UCI**: `openclash.@config_overwrite[0].enable_custom_clash_rules`
- **預設**: 0
- **說明**: 開啟後將在執行設定的 `rules:` 段注入自訂規則檔案中的內容
...
- **格式**: 每行一條 Mihomo 規則，插入到規則列表頂部（優先比對）
...
> **當使用者描述需求（如"我想讓某個網域名稱走代理"、"禁止某個 IP 走代理"）時，AI 應查閱 [Mihomo 路由規則文件](https://wiki.metacubex.one/config/rules/) 瞭解各規則型別的作用，涉及規則注入實現時查閱 [OpenClash 原始碼](https://github.com/vernesong/OpenClash/tree/dev) 中 `yml_rules_change.sh` 和 `custom_rules*.list` 的處理邏輯，然後告知使用者具體的規則寫法。**

**Mihomo 支援的規則型別速查**:

| 規則型別 | 格式 | 用途 | 範例 |
|---------|------|------|------|
| `DOMAIN` | `DOMAIN,網域名稱,策略` | 精確比對網域名稱 | `DOMAIN,www.google.com,Proxy` |
| `DOMAIN-SUFFIX` | `DOMAIN-SUFFIX,網域名稱字尾,策略` | 比對網域名稱字尾（含所有子網域名稱） | `DOMAIN-SUFFIX,google.com,Proxy` |
| `DOMAIN-KEYWORD` | `DOMAIN-KEYWORD,關鍵詞,策略` | 比對網域名稱含關鍵詞 | `DOMAIN-KEYWORD,youtube,Proxy` |
| `DOMAIN-REGEX` | `DOMAIN-REGEX,正則,策略` | 網域名稱正則比對 | `DOMAIN-REGEX,^api\.example\.com$,Proxy` |
| `GEOSITE` | `GEOSITE,類別,策略` | 按 GeoSite 類別比對網域名稱 | `GEOSITE,netflix,NETFLIX` |
| `GEOIP` | `GEOIP,國家程式碼,策略` | 按 GeoIP 國家比對 IP | `GEOIP,CN,DIRECT` |
| `IP-CIDR` | `IP-CIDR,IP/掩碼,策略` | IP 段比對 | `IP-CIDR,10.0.0.0/8,DIRECT` |
| `IP-CIDR6` | `IP-CIDR6,IPv6/掩碼,策略` | IPv6 段比對 | `IP-CIDR6,::1/128,DIRECT` |
| `IP-ASN` | `IP-ASN,ASN號,策略` | 自治系統號比對 | `IP-ASN,13335,Proxy` |
| `RULE-SET` | `RULE-SET,規則集名,策略` | 引用 rule-provider 規則集 | `RULE-SET,reject,REJECT` |
| `PROCESS-NAME` | `PROCESS-NAME,程式名,策略` | 按程式名比對 | `PROCESS-NAME,aria2c,DIRECT` |
| `DST-PORT` | `DST-PORT,埠,策略` | 目標埠比對 | `DST-PORT,80,Proxy` |
| `SRC-PORT` | `SRC-PORT,埠,策略` | 源埠比對 | `SRC-PORT,8080,DIRECT` |
| `SRC-IP-CIDR` | `SRC-IP-CIDR,IP/掩碼,策略` | 源 IP 段比對 | `SRC-IP-CIDR,192.168.1.0/24,DIRECT` |
| `MATCH` | `MATCH,策略` | 兜底比對所有流量 | `MATCH,Proxy` |

**可用策略目標**: `DIRECT`(直連)、`Proxy`(走預設代理組)、`REJECT`(拒絕)、`REJECT-DROP`(靜默丟棄)、`GLOBAL`(走全域性組)、任意自訂策略組名稱

**編寫格式**: 不區分大小寫，逗號分隔。每行一條規則。規則按順序從上到下比對，命中後不再繼續。
...
| 國內網域名稱直連 | `GEOSITE,cn,DIRECT` |
| Netflix 走專用策略組 | `GEOSITE,netflix,NETFLIX` |
| 禁止存取某網域名稱 | `DOMAIN-SUFFIX,badsite.com,REJECT` |
...
# 第四部分：設定訂閱頁面 (Config Subscribe / config-subscribe)

> UCI Section: `openclash.config_subscribe` (多條)

> **AI 行為指引**: 當使用者詢問訂閱相關問題（如"如何過濾節點"、"訂閱轉換怎麼用"、"訂閱 URL 格式不對怎麼辦"、
> "keyword 和 ex_keyword 的區別"、"Age 加密是什麼"），AI 應查閱 [Mihomo 代理協定文件](https://wiki.metacubex.one/config/proxies/)
...
> 然後告知使用者具體的設定方法。對於訂閱轉換後端問題，
> 告知使用者轉換後端的位址格式和模板 URL 的作用。
...
3. 如果設定了 `sub_convert`，將 URL 發到轉換後端取得處理後的設定
4. 如果設定了 `secret_key` (Age 加密)，先用 age 解密
5. 用 Ruby YAML 解析訂閱設定 → 取得所有代理節點
6. 根據 `keyword` / `ex_keyword` 正則比對過濾節點：
   - `&` = AND: 節點名必須同時包含所有關鍵字
   - `|` = OR: 節點名包含任一關鍵字即保留
7. 將過濾後的節點合併到當前設定的 `proxies` 和 `proxy-groups` 中
8. 寫入 `/etc/openclash/config/<name>.yaml`，標記核心需重啟

**關鍵字比對實現** (`server_key_match()`):
將使用者輸入的關鍵字轉換為 Ruby 正規表示式。`&` 分隔的轉為正向預查鏈 `(?=.*kw1)(?=.*kw2)`，`|` 分隔的轉為擇一比對 `(kw1|kw2)`。
...
| 訂閱位址 (Subscribe Address) | `address` | 訂閱 URL |
| **User-Agent** (UA) | `sub_ua` | 預設 clash-verge/clash.meta/clash |
| **線上訂閱轉換 (Subscribe Convert Online)** | `sub_convert` | 訂閱轉換後端位址 |
| **訂閱轉換模板 (Template Name)** | `sub_template` | 轉換模板 URL |
| **篩選節點 (Keyword Match)** | `keyword` | 節點關鍵字比對 (保留比對的節點) |
| **排除節點 (Exclude Keyword Match)** | `ex_keyword` | 排除關鍵字 (排除比對的節點) |
| **自訂引數 (Custom Params)** | `custom_params` | 自訂訂閱 URL 引數 |
...
# 第五部分：設定管理頁面 (Config Manage / config)

> LuCI 路徑: `服務` → `OpenClash` → `設定管理` (順序第 80)
> UCI 對映: `openclash.config.config_path`

## 實現總覽

設定管理頁面是一個多功能綜合頁面，提供設定檔案的上傳、切換、編輯、重新命名、刪除以及提供商檔案管理功能。

**核心功能區塊**:

| 區塊 | 功能 | 後端路由 |
|------|------|----------|
| 檔案上傳 (Upload) | 上傳設定檔案、代理/規則提供商、核心二進位制、備份恢復 | `/upload_config` + `file_type` 引數區分 |
| **設定檔案列表** | 檢視/切換/編輯/重新命名/複製/下載/刪除設定 | `/switch_config`、`/config_file_list`、`/config_file_save` 等 |
| **提供商檔案管理** | 跳轉到代理提供商和規則提供商管理子頁 | 跳轉連結 |
| **設定檔案編輯器** | 雙欄 YAML 編輯器（左側可編輯使用者設定，右側只讀預設模板） | `/config_file_read` + `/config_file_save` |

## 5.1 檔案上傳

| 上傳型別 | `file_type` 值 | 目標目錄 | 說明 |
|----------|---------------|----------|------|
| 設定檔案 (Config) | `config` | `/etc/openclash/config/` | `.yaml`/`.yml` 格式，上傳後自動設為當前啟用設定 |
...
| 備份恢復 | `backup-file` | 恢復到 `/etc/config/openclash` | 上傳備份並恢復 UCI 設定 |

## 5.2 設定檔案列表

| 操作 | 功能 | 說明 |
|------|------|------|
| **SwiTch** (切換) | 切換啟用設定 | 修改 `config_path` UCI + commit，自動重啟核心 |
| **Edit** (編輯) | 線上編輯設定 | 跳轉到雙欄 YAML 編輯器 |
| **Rename** (重新命名) | 重新命名 | 輸入新名稱，`mv` 重新命名檔案 |
| **Copy** (複製設定) | 複製設定 | 產生 `<檔名>(N).yaml` 副本 |
| **Download** (下載設定) | 下載設定檔案 | HTTP 下載原始設定檔案 |
| **Download Run** (下載執行設定) | 下載執行時設定 | 下載 `/etc/openclash/<name>`（經指令碼處理後的實際執行設定） |
| **Remove** (移除) | 刪除設定 | 刪除 YAML 檔案 + 歷史快取 `/etc/openclash/history/<name>.db` + 執行時設定，自動切換到其他設定 |

## 5.3 設定檔案編輯器

雙欄 YAML 編輯器：
- **左欄 (可編輯)**: 讀寫當前 `config_path` 指向的設定，儲存時自動 `\r\n` → `\n` 轉換
- **右欄 (只讀)**: 展示執行時設定 `/etc/openclash/<name>` 或預設模板 `/usr/share/openclash/res/default.yaml`
- **操作按鈕**: Commit (儲存設定 (Commit Settings))、Create (新建設定)、Apply (應用設定 (Apply Settings))
- **快捷鍵**: F10 diff 控制、F11 全屏模式

## 5.4 提供商子頁面

透過設定管理頁面可跳轉到以下子頁面（獨立 CBI 頁面）：
- **servers** — 代理節點管理（編輯/新增/刪除節點）
- **servers-config** — 節點設定編輯器
- **groups-config** — 策略組設定編輯器
- **proxy-provider-config** — 代理提供商設定
...
**標籤頁 3 — Debug Logs** (可切換)：展示外掛的除錯日誌，內容由 `openclash_debug.sh` 產生，包含系統資訊、相依套件檢查、核心執行狀態、外掛設定、覆寫模組設定、自訂規則檔案內容、當前 Mihomo YAML 設定、自訂覆寫/防火牆指令碼內容、完整的 iptables-save dump、完整的 nftables 規則、ipset 狀態、路由表、TUN 裝置狀態、埠占用、DNS 解析測試、網路連通性測試、最近執行日誌、活動連線列表及隱私處理等。
...
| **Generate Logs** (產生除錯日誌) | 點選產生除錯日誌並展示 | 前端下載時不拼接 |
...
**除錯日誌來源|實現細節**: `openclash_debug.sh` 使用檔案鎖防止併發，收集以下 20+ 個章節並輸出到 `/tmp/openclash_debug.log`：
  ...
  2) 相依套件檢查（dnsmasq-full、bash、curl、ruby、ruby-yaml、kmod-tun、kmod-inet-diag、kmod-nft-tproxy 或 kmod-ipt-tproxy 等）
  3) 核心執行狀態（PID、執行使用者、Meta 核心版本 `clash_meta -v`）
  4) 外掛設定（所有 UCI 執行模式/代理/DNS/IPv6 設定值）
  5) 覆寫模組設定（`uci show openclash.@overwrite[0]`）
  6) 自訂規則檔案內容
  7) 當前 Mihomo YAML 設定（過濾掉 proxies/proxy-providers/secret 保護隱私）
  8) 自訂覆寫/防火牆指令碼內容
  ...
  18) 活動連線列表（透過 Mihomo API 取得）
  19) 隱私處理（IPv4 最後一位元組和 IPv6 後半部分模糊化）

**附加元件**: 頁面同時載入 `openclash/toolbar_show`（**設定切換工具欄**：下拉選擇當前設定檔案 + Switch 按鈕）和 `openclash/config_editor`（**頁面內嵌 CodeMirror 編輯器**，預載入 CodeMirror CSS/JS 資源並透過全域性 `merge_editor()` 函式對外開放合併檢視功能，非日誌渲染用途，僅作檔案編輯功能複用）。
...
> **用途**: 當使用者描述問題但缺少關鍵資訊時，AI 應給出精確的 SSH 指令讓使用者在路由器上執行，
> 然後根據使用者返回的輸出結果進行診斷。
>
> **互動模式**: AI 給出指令 → 使用者複製到路由器終端執行 → 使用者貼上輸出 → AI 分析並決定下一步。
> 指令按安全等級標註：🟢 安全查詢 / 🟡 有副作用 / 🔴 高風險。AI 應優先推薦 🟢 指令，
> 對 🟡/🔴 指令應附帶風險說明。
...
> AI 根據使用者症狀選擇對應子節，按步驟順序執行指令，每步根據輸出決定下一步。
...
| 3. dnsmasq 設定 | `uci show dhcp.@dnsmasq[0] \| grep -E 'server\|noresolv'` | 🟢 | `server=127.0.0.1#7874` | 非此→DNS 轉發鏈路斷開 |
| 4. 解析測試 | `nslookup www.google.com 127.0.0.1` | 🟢 | Fake-IP 模式返回 `198.18.x.x` | 返回真實IP→Fake-IP 未生效；無回應→服務異常 |
| 5. dnsmasq | `dnsmasq --version \| head -1` | 🟢 | 含 `full` 或 ipset/nftset | 精簡版→換 `dnsmasq-full` |

### 7.1.3 存取控制問題（某裝置不走/走了代理）
...
| 步驟 | 指令 | 安全 | 期望輸出 | 異常處理 |
|------|------|------|----------|----------|
| 1. 節點狀態 | `curl -s http://127.0.0.1:9090/proxies \| grep -E '"name"\|"history"' \| head -20` | 🟢 | 含延遲資料 | 全部逾時→檢查網路/節點可用性 |
...
| 2. UCI enable | `uci get openclash.@openclash[0].enable` | 🟢 | `1` | `0`→外掛被停用 |
...
| 6. 除錯日誌 | `/usr/share/openclash/openclash_debug.sh` | 🟡 | 產生 `/tmp/openclash_debug.log` | 日誌含 `#===== 依賴檢查 =====#` 段 |
...
| `uci show openclash \| grep <key>` | 查特定 UCI 設定 |
...
| `/usr/share/openclash/openclash_debug.sh` | 產生除錯日誌 | 寫 `/tmp/openclash_debug.log` |
...
| `curl -X PATCH http://127.0.0.1:9090/configs -d '{"mode":"rule"}'` | 熱切換代理模式 | 立即影響所有用戶端 |
...
| `/usr/share/openclash/openclash.sh <name>` | 更新訂閱 | 替換設定+自動重啟 |
...
> 以下指令在路由器終端直接執行（本機 `127.0.0.1`，無需認證）。
> 返回 JSON 格式，可追加 `| jsonfilter -e '@.key'` 提取特定欄位（OpenWrt 內建工具）。

### 狀態查詢

```bash
# 執行狀態總覽（核心狀態、Dashboard位址、埠、核心型別）
...
# 本機設定與已裝版本（編譯版本 corever、釋出分支 release_branch、Smart 狀態 smart_enable、oix/pkg_type、CPU 架構 coremodel、已安裝外掛/核心版本 opcv/coremetacv；不含遠端最新）
...
# 訂閱流量/到期資訊（替換 <設定名>）
curl -s 'http://127.0.0.1/cgi-bin/luci/admin/services/openclash/sub_info_get?filename=<設定名>'
...
| 欄位 | 含義 | 正常值 | 異常含義 |
|------|------|--------|----------|
| `clash` | UCI enable 開關 | `true` | `false` → 外掛被停用，需在「執行狀態」頁開啟 |
...
| `meta_sniffer` | 網域名稱嗅探 | `"1"` | `"0"` → 嗅探關閉，網域名稱規則可能失效 |
...
| 欄位 | 來源 | 含義 |
|------|------|------|
| `corelv` | Lua `fetch_version_history` 快取 | 遠端最新核心版本，`"loading..."`=尚未取得 |
...
| `day_left` | 剩餘天數 | `0` → 已過期；`"null"` → 無法取得；`"∞"` → 長期有效 |
...
# --- 設定與訂閱 ---
# 切換設定檔案（替換 <檔名.yaml>，會自動重啟核心）
curl -s -X POST -d 'config_file=<檔名.yaml>' http://127.0.0.1/cgi-bin/luci/admin/services/openclash/switch_config
# 更新指定訂閱設定（替換 <設定名>）
curl -s -X POST -d 'filename=<設定名>' http://127.0.0.1/cgi-bin/luci/admin/services/openclash/update_config

# --- 更新操作 ---
# 更新核心（按 uci 設定構建下載 URL，無完整 URL）
...
> 以下指令在路由器終端直接執行，核心必須執行中。
> 如設定了 `dashboard_password`，所有指令需追加 `-H "Authorization: Bearer <password>"`。
...
curl -s -X PUT -H 'Content-Type: application/json' \
  -d '{"path":"/etc/openclash/<設定檔名>.yaml"}' \
...
# 關閉指定連線（替換 <連線ID>，ID 從 /connections 返回中取得）
...
| `.proxies.<節點名>.history[]` | 最近延遲記錄 (`{"delay":<ms>,"time":"..."}`)，`delay=0` 表示逾時 |
...
| `.connections[].rule` | 比對的規則型別 (如 `DOMAIN-SUFFIX,google.com`) |
| `.connections[]. metadata.host` | 目標網域名稱 |
| `.uploadTotal` / `.downloadTotal` | 累計上下行流量（位元組） |

> 診斷提示：若連線列表為空但有網路活動 → 流量未進入核心（防火牆規則問題）；若 `chains` 全部為 `["DIRECT"]` → 規則比對為直連。
...
```bash
# --- 診斷 ---
# 產生完整除錯日誌（輸出到 /tmp/openclash_debug.log，含依賴檢查、防火牆規則、設定等 20+ 章節）
...
# 更新單個訂閱設定（替換 <設定檔名>，不含路徑和副檔名）
/usr/share/openclash/openclash.sh <設定檔名>

# 下載並替換 Meta 核心（使用預設 GitHub 位址）
...
| `openclash_custom_domain_dns.sh` | init.d | 自訂網域名稱 DNS |
| `openclash_debug_dns.lua` | Web UI | DNS 解析測試 |
| `openclash_debug_getcon.lua` | Web UI | 活動連線取得 |
| `openclash_streaming_unlock.lua` | 看門狗 | 流媒體解鎖切換 |
| `openclash_sub_parser.lua` | 看門狗 | 訂閱格式解析 |

## Mihomo Wiki 參考連結

- [全域性設定 (General)](https://wiki.metacubex.one/config/general/)
- [DNS 設定](https://wiki.metacubex.one/config/dns/)
- [TUN 設定](https://wiki.metacubex.one/config/inbound/tun/)
- [網域名稱嗅探 (Sniffer)](https://wiki.metacubex.one/config/sniff/)
- [路由規則](https://wiki.metacubex.one/config/rules/)
- [代理協定](https://wiki.metacubex.one/config/proxies/)
- [完整設定範例](https://github.com/MetaCubeX/mihomo/blob/Meta/docs/config.yaml)

---

# 第八部分：覆寫模組詳解

> 覆寫模組 (Overwrite Module) 是 OpenClash 的高階自訂功能
> **主入口（使用者詢問"怎麼用"時優先講這個）**: 執行狀態頁頂部**「覆寫模組」按鈕**（`id="edit_overwrite"`，與啟動/停止開關並列，呼叫 `editOverwrite()`）——點選彈出覆寫編輯器視窗，建立/編輯/刪除/啟停覆寫模組都在這一個視窗內完成。
> 次要入口: 選單 `服務→OpenClash→覆寫設定`（獨立 CBI 頁面，設定的是內建覆寫選項，與「覆寫模組」檔案編輯是不同入口，見第三部分）
> UCI Section: `openclash.config_overwrite` (支援多條，按 order 排序)
> 覆寫檔案儲存: `/etc/openclash/overwrite/<名稱>` (本機) 或透過 HTTP 遠端拉取；內建固定檔案 `/etc/openclash/custom/openclash_custom_overwrite.sh`

## 8.1 覆寫模組是什麼

> **AI 行為指引**: 當使用者詢問覆寫模組相關問題（如"覆寫模組怎麼用"、"如何透過覆寫新增設定"、"[YAML] 運算子怎麼用"、
...
>    **AI 輸出任何覆寫設定範例時，必須在程式碼塊內以段頭作為第一行**，**絕對禁止**輸出不含段頭的裸 YAML/Shell/INI 內容。
>    即使使用者只問「某個欄位怎麼寫」，程式碼塊也必須形如：
>    ```ini
>    [YAML]
>    <具體設定>
>    ```
>    而非僅 `<具體設定>`。若使用者反饋覆寫不生效，優先排查：①段頭是否存在；② `config` 欄位是否比對當前設定檔案。
>
> 2. **範例輸出規範**：優先使用 `[YAML]` 段格式給出範例（語法清晰、不易出錯）；僅當需要動態邏輯（條件判斷、迴圈處理）時才推薦 `[Overwrite]` 段。
>    給出範例前應**先明確使用者需求**（追加還是替換？鍵路徑是什麼？目標是陣列還是雜湊？比對條件？），然後結合 [8.2.3 節運算子]（`!` 強制覆蓋 / `+` 陣列追加 / `-` 陣列刪除 / `*` 批次條件更新等）給出精準的、可直接使用的設定片段。禁止給出不含段頭的泛泛描述。
>
> 3. **資訊取得路徑**：本章節未覆蓋的細節按以下優先順序查閱——
>    - 覆寫檔案格式/運算子/範例 → 本章節（8.2 格式說明、8.2.3 運算子、8.5 實戰範例）
>    - Mihomo YAML 欄位含義/用法 → [Mihomo 設定文件](https://wiki.metacubex.one/config/)
>    - 覆寫執行機制/排序/指令碼邏輯 → [OpenClash 原始碼](https://github.com/vernesong/OpenClash/tree/dev) 中 `init.d/openclash` 的 `overwrite_file()` 函式和 `/tmp/yaml_overwrite.sh` 產生邏輯
>
...
- 觸發：`editOverwrite()` → `ConfigEditor.showOverwrite()`（`config_edit.htm`），複用設定編輯器模態框。與選單「服務→OpenClash→覆寫設定」CBI 頁面是**兩個獨立入口**。
...
- 兩個標籤頁：
  - **File**：直接新建本機覆寫檔案——填「檔名 / 比對設定檔案（config：`all` 或指定檔名）/ 順序（order）」→ Add。
...
**④ 編輯（語法格式與儲存）**
- 點選卡片（或齒輪）→ 在主編輯器開啟該覆寫檔案，按 **INI 三段格式**編輯：`[General]`（鍵值對/環境變數）、`[Overwrite]`（Shell 指令，可用 `ruby_*` 函式族）、`[YAML]`（原始 YAML + 運算子）。**必須包含至少一個段頭**，否則不生效。詳細格式/運算子見 8.2。
...
**⑤ 原理（生效機制）**
- 覆寫檔案落盤 `/etc/openclash/overwrite/<名稱>`，並註冊到 UCI `openclash.config_overwrite`（按 order 排序、config 比對當前設定）。
- 重啟 OpenClash 時 `overwrite_file()`（`init.d/openclash`）按段頭解析：`[General]` 提前寫入 UCI（影響 `yml_change.sh` 行為）；`[Overwrite]`/`[YAML]` 產生 `/tmp/yaml_overwrite.sh`，在 `yml_change.sh`/`yml_rules_change.sh` **之後**執行 → 深度合併/覆蓋訂閱與 LuCI 輸出（含硬編碼項，覆蓋需謹慎）。

> **注意**：以上是「覆寫模組」（檔案式自訂）的操作方式。選單「覆寫設定」CBI 頁（第三部分）設定的是內建覆寫選項（DNS/規則/Smart 等 UCI 選項）；`yml_change.sh` 的覆寫邏輯是實現細節——兩者僅在使用者追問時補充，不作為「怎麼用」的主線。
...
2. 檢查覆寫是否比對當前設定檔案（`config` 欄位支援 `all` 或指定檔名）
...
6. 產生 `/tmp/yaml_overwrite.sh` 指令碼（包含 `[Overwrite]` 和 `[YAML]` 段的內容，暫不執行）

**第二階段 — YAML 覆寫**（`/tmp/yaml_overwrite.sh`，在 `yml_change.sh` 和 `yml_rules_change.sh` 之後執行）：
7. 執行 `[Overwrite]` 段的 Shell 指令（可使用 `ruby_*` 函式族修改 YAML）
8. 將 `[YAML]` 段的 YAML 內容深度合併到執行設定

> **執行順序含義**：`[General]` 段在 `yml_change.sh` 之前生效（因為寫入 UCI），因此可以影響 `yml_change.sh` 的行為；`[Overwrite]` 和 `[YAML]` 段在 `yml_change.sh` 和 `yml_rules_change.sh` 之後執行，因此**可以覆蓋這兩個指令碼的所有輸出**——包括「外掛強制覆蓋/停用的設定」表格中的硬編碼項（如 `allow-lan`、`bind-address`、`sniffer.sniff` 等）。⚠️ **覆蓋這些硬編碼項可能導致功能異常**，請謹慎使用。

**覆寫模組能做什麼**:
- 給訂閱設定**追加/覆蓋**任意 Mihomo YAML 欄位（如 DNS、Sniffer、TUN、規則等）
- 設定環境變數供 `yml_change.sh` 等後續指令碼使用
- 下載外部檔案（透過 `DOWNLOAD_FILE` 指令）
- 對未提供 UI 選項的 Mihomo 高階功能進行設定
...
| 自訂 | `ENABLE_CUSTOM_CLASH_RULES`, `ENABLE_RULE_PROXY` | int_bool | 規則 |
| Smart | `AUTO_SMART_SWITCH`, `SMART_ENABLE_LGBM`, `SMART_POLICY_PRIORITY` | int_bool/string | Smart 策略 |
| 特殊 | `CONFIG_FILE` | string | 覆寫 config_path（切換設定） |
...
> 這些環境變數在 `yml_change.sh`、`yml_rules_change.sh` 及自訂覆寫指令碼中可透過 `$KEY_NAME` 直接引用。

### 8.2.2 `[Overwrite]` 段 — Shell 指令碼

此段內容直接作為 Shell 指令執行。可用的函式：
...
- `uci_get_config <key>` — 讀取 UCI 設定（覆寫優先）

### 8.2.3 `[YAML]` 段 — 原始 YAML 注入（含運算子）

`[YAML]` 段使用 Ruby 將內容**深度合併**到執行設定檔案。支援多種**運算子字尾**實現精細控制：
...
| **批次條件更新** | `key*` 或 `<key>*` | 按 `where` 條件比對，用 `set` 子句更新（見下） |
...
按 `where` 條件比對集合元素，用 `set` 子句更新指定欄位。
...
| 型別 | UCI `type` 值 | 說明 |
|------|--------------|------|
| **本機檔案** | `file` | 讀取 `/etc/openclash/overwrite/<名稱>` |
...
每個覆寫條目可指定目標設定檔案（`config` 欄位，ListValue）:
- `all` — 對所有設定檔案生效
- `/etc/openclash/config/xxx.yaml` — 僅對該設定檔案生效

## 8.5 實戰範例

### 範例1: 強制啟用 TUN 模式 + 設定 DNS
```ini
[General]
EN_MODE = fake-ip-tun
STACK_TYPE = mixed
...
| `type` | string | `file` | `file`=本機檔案；`http`=遠端下載（需設定 `url`/`update_days`/`update_hour`） |
| `url` | string | *(空)* | `type=http` 時的下載連結 |
| `config` | ListValue | *(空)* | 目標設定檔案列表。`all`=應用到所有設定；或指定具體路徑如 `/etc/openclash/config/xx.yaml`。**為空則永不比對，覆寫不生效** |
...
**`config` (目標設定)**:
- 覆寫條目**必須**透過此欄位比對當前執行的設定檔案才會執行。比對邏輯（`overwrite_config_match_check()`）：
  - `config` 列表包含 `all` → 比對所有設定
  - `config` 列表包含當前 `config_path` UCI 值 → 比對
  - `config` 為空 → **永不比對，覆寫不生效**（常見設定錯誤）
- 支援同時比對多個設定檔案。
...
| `URLTEST_ADDRESS_MOD` | string | `urltest_address_mod` | 測速位址 |
| `URLTEST_INTERVAL_MOD` | int | `urltest_interval_mod` | 測速間隔 |
| `GITHUB_ADDRESS_MOD` | string | `github_address_mod` | GitHub CDN 位址 |
| `ENABLE_REDIRECT_DNS` | int_bool | `enable_redirect_dns` | DNS 劫持模式 |
| `ENABLE_CUSTOM_DNS` | int_bool | `enable_custom_dns` | 自訂 DNS |
| `ENABLE_RESPECT_RULES` | int_bool | `enable_respect_rules` | DNS 尊重規則 |
| `ENABLE_META_SNIFFER` | int_bool | `enable_meta_sniffer` | 網域名稱嗅探 |
| `ENABLE_META_SNIFFER_PURE_IP` | int_bool | `enable_meta_sniffer_pure_ip` | 純 IP 嗅探 |
| `ENABLE_META_SNIFFER_CUSTOM` | int_bool | `enable_meta_sniffer_custom` | 自訂嗅探 |
...
| `DISABLE_UDP_QUIC` | int_bool | `disable_udp_quic` | 停用 QUIC |
| `DISABLE_QUIC_GO_GSO` | int_bool | `disable_quic_go_gso` | 停用 quic-go GSO |
| `FIND_PROCESS_MODE` | string | `find_process_mode` | 程式比對模式 |
...
| `SKIP_PROXY_ADDRESS` | int_bool | `skip_proxy_address` | 繞過伺服器位址 |
...
| `CUSTOM_HOST` | int_bool | `custom_host` | 自訂 Hosts |
...
| `GEO_CUSTOM_URL` | string | `geo_custom_url` | MMDB 自訂 URL |
...
| `GEOIP_CUSTOM_URL` | string | `geoip_custom_url` | Dat 自訂 URL |
...
| `GEOSITE_CUSTOM_URL` | string | `geosite_custom_url` | GeoSite 自訂 URL |
...
| `GEOASN_CUSTOM_URL` | string | `geoasn_custom_url` | ASN 自訂 URL |
...
| `LGBM_CUSTOM_URL` | string | `lgbm_custom_url` | LGBM 自訂 URL |
| `LGBM_UPDATE_INTERVAL` | int | `lgbm_update_interval` | LGBM 更新間隔 |
| **規則類** | | | |
| `ENABLE_CUSTOM_CLASH_RULES` | int_bool | `enable_custom_clash_rules` | 自訂規則 |
...
> **強制規則：當使用者詢問本文件未覆蓋的 Mihomo/OpenClash 設定或實現細節時，禁止自行猜測或編造回答。
> AI 必須主動查詢外部資源取得準確資訊後回覆使用者。**

本文件僅覆蓋 OpenClash LuCI 外掛 UI 中可直接設定的選項及其實現。當遇到本文件未覆蓋的問題時，AI 必須**主動**使用以下資源查詢答案，而非讓使用者自己去查閱文件：

**AI 必須主動查詢的外部資源**：

| 優先順序 | 資源 | 查詢方式 | 適用場景 |
|--------|------|----------|----------|
| 1 | **Mihomo Wiki** `https://wiki.metacubex.one/config/` | 使用 `fetch_webpage` 抓取相關頁面 | Mihomo YAML 設定欄位的含義、可選值、用法 |
| 2 | **Meta-Docs 倉庫** `github.com/MetaCubeX/Meta-Docs` | 使用 `github_text_search` 搜尋 `docs/config/` 目錄 | 需要精確的欄位型別、預設值、完整設定範例 |
| 3 | **OpenClash Issues** `https://github.com/vernesong/OpenClash/issues` | 使用 `fetch_webpage` 開啟 Issue 搜尋頁面或具體 Issue 頁面 | 外掛側功能異常/報錯（設定/訂閱/防火牆/UI等），搜尋已知問題和社群方案（優先作者 vernesong 回覆和高贊回答） |
| 4 | **Mihomo Issues** `https://github.com/MetaCubeX/mihomo/issues` | 使用 `fetch_webpage` 開啟 Issue 搜尋頁面或具體 Issue 頁面 | 核心側功能異常/報錯（代理協定/TUN/DNS解析/規則引擎等），搜尋已知問題和社群方案 |
| 5 | **OpenClash 原始碼** `github.com/vernesong/OpenClash/tree/dev` | 使用 `github_text_search` 搜尋對應指令碼/函式 | 需要了解外掛側的實現邏輯、UCI 到 YAML 的轉換細節 |
| 6 | **Mihomo 核心原始碼** `github.com/MetaCubeX/mihomo/tree/Alpha` | 使用 `github_text_search` 搜尋核心程式碼 | 代理協定實現、規則引擎、DNS 解析、TUN 棧等核心底層邏輯 |
| 7 | **Smart 核心原始碼** `github.com/vernesong/mihomo/tree/Alpha` | 使用 `github_text_search` 搜尋核心程式碼 | Smart 策略、LightGBM 模型的底層實現 |

**具體觸發條件**（滿足任一即主動查詢）：
- 使用者詢問的設定欄位在本文件任何章節中均未出現
- 使用者詢問特定代理協定的詳細引數（Hysteria2/TUIC/WireGuard/SSH/MASQUE 等的完整 TLS/傳輸層選項）
- 使用者詢問 `experimental`、`tunnel`、`sub-rule` 等外掛 UI 中無直接對應選項的 Mihomo 頂級設定段
- 使用者需要編寫超出覆寫模組 8.2 節範例範圍的自訂指令碼或 YAML 設定
...
1. 確認問題超出本文件覆蓋範圍
2. 根據問題型別選擇對應的外部資源
3. **優先搜尋 Issues**：如果使用者遇到的是功能異常/報錯類問題（而非設定欄位查詢），應先搜尋 Issues 查詢類似問題。根據問題型別選擇：外掛側（設定/訂閱/防火牆/UI）→ [OpenClash Issues](https://github.com/vernesong/OpenClash/issues)；核心側（代理協定/TUN/DNS/規則引擎）→ [Mihomo Issues](https://github.com/MetaCubeX/mihomo/issues)。讀取 Issue 時重點關注：① 維護者的診斷指令和結論；② 👍 反應數高的社群回覆；③ Issue 最終是否被關閉及關閉原因（`completed`=已修復，`not planned`=不在計劃內）
